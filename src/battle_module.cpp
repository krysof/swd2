...
BattleSurface compose_introduction(
    ...
    std::optional<std::size_t> cursor_marker = std::nullopt) {
    auto result = scene;
    ...
    if (cursor_marker) {
        blit(result, menu_sprites, *cursor_marker,
...
bool present_round_events(
    ...
    for (std::size_t event_index = 0; event_index < result.events.size();
        ...
        const auto composite_dispatcher_continues =
            retained_fading_handler &&
            event_index + 1U < result.events.size() &&
            fig_same_presented_action(
                event, result.events[event_index + 1U]) &&
            event.effect_code != result.events[event_index + 1U].effect_code &&
            is_player_dispatcher_handler(result.events[event_index + 1U]);
        ...
        if (player_dispatcher_action &&
            retained_composite_dispatcher_palette) {
            // 57f2 keeps the first nested handler's dark DAC table live for
            // every second selector, not only damage->damage/status->damage
            // pairs.  The intermediate handler neither pays/restores nor
            // repeats 43ce; only the final nested handler reaches 585e/4417.
            dispatcher_palette_override =
                *retained_composite_dispatcher_palette;
        ...
        if (retained_player_dispel_handler) {
            ...
            // steps. The ordinary player caller next enters 0c41: 0d98 owns
            // the bare five-tick page only when no same-turn expiry first
            // enters 0da7 over the retained dispel surface.
            apply_visual_event(visual, event, abilities);
            present_player_resource_cost(event);
            for (auto step = 0; step < 5; ++step) {
                brighten_fig_dispatcher_palette(
        ...
        if (retained_monster_status_handler) {
            ...
            present_battle_surface(context, restored);
            if (composite_dispatcher_continues) {
...
Marker BattleModule::run(GameContext& context, Marker input) {
    ...
    if (encounter.introduction_text.empty()) {
        // Empty introductions branch directly to 2b64, so the first host page
        // already has the party cards installed. A real 3de8 introduction,
        // however, starts from the bare 2deb enemy page and does not install
        // these cards until the command compositor after the text/prompt.
    ...
    if (!encounter.introduction_text.empty()) {
        ...
        while (true) {
            ...
            const auto wait_at_text_cursor = [&](bool animated) {
                ...
                while (true) {
                    auto frame = compose_introduction(
                        surface, page, menu_sprites, std::nullopt, marker);
                    context.platform.present({
                        320, 200, frame.pixels,
                        std::span<const std::uint8_t, 768>(frame.palette),
                    });
                    ...
                    if (input != InputAction::none) {
                        return CursorWaitResult{input, marker};
                    }
            ...
            if (page.has_more) {
                ...
                action = wait_at_text_cursor(false).action;
            ...
            // 3ed3 cycles MENU 95h..98h at every final $$ until any key.
            // DATA:2a40 starts cleared and has no writer in the shipped FIG
            // image, so its nominal bypass is unreachable even for trailing
            // YN/NY records. 5c98 then draws the prompt over the last cursor
            // frame rather than erasing the marker.
            const auto final_wait = wait_at_text_cursor(true);
            action = final_wait.action;

...
void apply_rpg_event_monochrome_filter(
    ...
    for (std::size_t color = 0; color < translation.size(); ++color) {
        // RPG:0dbf:0314 deliberately leaves the reserved 10h..1fh
        // inverse-luminance ramp untouched.  Every other source colour is
        // reduced through the saved VGA palette into that ramp.
        if (color >= 0x10U && color < 0x20U) {
            translation[color] = static_cast<std::uint8_t>(color);
            continue;
        }
        ...
        translation[color] = static_cast<std::uint8_t>(0x1fU - luminance);
    }

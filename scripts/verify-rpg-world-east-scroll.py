#!/usr/bin/env python3
"""Replay and lock AREA1's original four-cell east viewport scroll."""
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed AREA1 east-scroll digest {label}")
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_rpg_area1_east_scroll_sequence" or \
                expected.get("status") != "exact_rgb_checkpoint" or \
                expected.get("initial_world_position") != [126, 13] or \
                expected.get("requested_direction") != "RIGHT" or \
                expected.get("direction_polls") != 4 or \
                expected.get("final_world_position") != [130, 13] or \
                expected.get("initial_viewport_position") != [106, 1] or \
                expected.get("final_viewport_position") != [110, 1] or \
                not isinstance(pages, list) or len(pages) != 3 or \
                tuple(page.get("kind") for page in pages) != (
                    "initial_release_area1_world_page",
                    "second_east_scroll_page",
                    "third_east_scroll_page") or \
                tuple(page.get("rewrite_frame") for page in pages) != \
                    (0, 2, 3) or \
                tuple(page.get("original_review_frame") for page in pages) != \
                    (299, 535, 768):
            raise ValueError("unsupported RPG AREA1 east-scroll reference")
        if sha256((args.game / "RPG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError("RPG.EXE differs from east-scroll reference")
        if tuple(sha256((args.game / "T1" / name).read_bytes()) for name in
            ...
            raise ValueError("AREA1 resources differ from east-scroll reference")
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
                ...
                sha256((args.save_root / "NAMEQ.DSK").read_bytes()) != \
                    NAME_SHA256:
            raise ValueError("RPG AREA1 east-scroll save triplets differ")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("RPG AREA1 east-scroll input evidence differs")
        if expected.get("capture_wait_seconds") != 5 or \
            ...
            raise ValueError("RPG AREA1 east-scroll capture boundary differs")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("RPG AREA1 east-scroll replay timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    expected["rewrite_mapz_fnv1a64"],
                    expected["rewrite_name_fnv1a64"]):
            raise ValueError("RPG AREA1 east-scroll final state differs")
        if trace.get("transitions") != [{
                "module": "RPG.EXE", "input": "OC", "output": "--",
                "launched": True}]:
            raise ValueError("RPG AREA1 east-scroll replay missed OC entry")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 5:
            raise ValueError("RPG AREA1 east-scroll frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "RPG AREA1 east-scroll page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")
        print(
            "RPG AREA1 east scroll: released world (126,13), four east "
            "viewport steps to (130,13), three full RGB checkpoints match "
            "the untouched original")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"RPG AREA1 east scroll: FAIL: {error}\n")

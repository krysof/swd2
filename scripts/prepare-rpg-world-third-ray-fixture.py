#!/usr/bin/env python3
"""Stage BUIN1's released third-ray interaction entity in isolation."""
...
EXPECTED_SAVE_SHA256 = \
    "91d24dc8049641730b9c7c83f83db04bea818e9f687435beed7e99e88a827e4c"
...
def main() -> int:
    ...
    try:
        ...
        if actual_mapz != EXPECTED_MAPZ_SHA256:
            raise ValueError(f"third-ray MAPZ differs: {actual_mapz}")
        ...
        # (18,170), face west and issue Confirm: the centre and south rays
        # remain empty, then the third ray north finds the entity.
        for offset, value in (
                (0x408, 96), (0x40d, 55808), (0x40f, 8), (0x424, 268),
                (0x012, 34), (0x02a, 104), (0x0a2, 6),
                (0x41b, 0), (0x41d, 155)):
        ...
        if actual_save != EXPECTED_SAVE_SHA256:
            raise ValueError(f"third-ray SAVE differs: {actual_save}")
        ...
        print(f"RPG BUIN1 third-ray fixture: SAVE {actual_save}, MAPZ {actual_mapz}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(1, f"RPG BUIN1 third-ray fixture: FAIL: {error}\n")

#!/usr/bin/env python3
"""Replay and prove FIG's generic YN introduction and prompt boundary."""
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != "original_fig_introduction_yn_prompt_1e8" or \
                expected.get("formation_directory_offset") != 0x1E8 or \
                not isinstance(pages, list) or len(pages) != 7 or \
                [page.get("rewrite_frame") for page in pages] != rewrite_frames:
            raise ValueError("unsupported FIG YN-introduction reference")
        ...
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("FIG 1e8h introduction staged save differs")
        ...
        subprocess.run([
            ...
            str(args.reference.with_name("replay-fig-introduction-yn-1e8.txt")),
        ...
        if trace.get("input") != {
                ...
                "implicit_quit_calls": 0} or trace.get("boundaries") != {
                    "wait": 2, "poll": 4, "text": 1, "frontend": 0}:
            raise ValueError("FIG 1e8h introduction input boundaries differ")
        if trace.get("video") != {
                "frames": 9, "direct_updates": 2,
                "last_width": 320, "last_height": 200,
                "fnv1a64": expected["rewrite_video_fnv1a64"]}:
            raise ValueError("FIG 1e8h introduction video differs")
        if trace.get("audio") != {
            ...
            raise ValueError("FIG 1e8h introduction audio or timing differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("FIG 1e8h introduction final state differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "OC",
                "launched": True}]:
            raise ValueError("FIG 1e8h introduction return boundary differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 9:
            raise ValueError("FIG 1e8h introduction frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    f"FIG 1e8h introduction {page['kind']} no longer matches original")
        ...
        print(
            "FIG 1e8h YN introduction: bare enemy page, first glyph, all four "
            "animated cursor phases, the default prompt, and seven exact "
            "original RGB pages; the unobserved right selection is not claimed")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG 1e8h YN introduction: FAIL: {error}\n")

#!/usr/bin/env python3
"""Replay and prove FIG's seven-page introduction without a prompt."""
...
def main() -> int:
    ...
    try:
        ...
        rewrite_frames = [0, 1] + list(range(3, 19))
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != "original_fig_seven_page_introduction" or \
                expected.get("formation_directory_offset") != 0x03A or \
                expected.get("page_count") != 7 or \
                not isinstance(pages, list) or len(pages) != 18 or \
                [page.get("rewrite_frame") for page in pages] != rewrite_frames:
            raise ValueError("unsupported FIG multipage introduction reference")
        ...
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("FIG introduction staged save differs")
        ...
        subprocess.run([
            ...
            str(args.reference.with_name("replay-fig-introduction-multipage.txt")),
        ...
        if trace.get("input") != {
                ...
                "implicit_quit_calls": 0} or trace.get("boundaries") != {
                    "wait": 1, "poll": 10, "text": 7, "frontend": 0}:
            raise ValueError("FIG introduction input boundaries differ")
        if trace.get("video") != {
                "frames": 26, "direct_updates": 14,
        ...
        if trace.get("audio") != {
                ...
                trace.get("delay_milliseconds") != 444:
            raise ValueError("FIG introduction audio or timing differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("FIG introduction final state differs")
        ...
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    f"FIG introduction {page['kind']} no longer matches original")
        ...
        print(
            "FIG multipage introduction: seven modern pages and eighteen exact "
            "original RGB pages through text page five")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG multipage introduction: FAIL: {error}\n")

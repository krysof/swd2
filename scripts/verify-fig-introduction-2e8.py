#!/usr/bin/env python3
"""Replay and prove FIG's two-page 2e8h introduction through commands."""
...
def main() -> int:
    ...
    try:
        ...
        rewrite_frames = [0, 1] + list(range(3, 11))
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != "original_fig_two_page_introduction_2e8" or \
                expected.get("formation_directory_offset") != 0x2E8 or \
                expected.get("page_count") != 2 or \
                not isinstance(pages, list) or len(pages) != 10 or \
                [page.get("rewrite_frame") for page in pages] != rewrite_frames:
            raise ValueError("unsupported FIG two-page introduction reference")
        ...
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("FIG 2e8h introduction staged save differs")
        ...
        subprocess.run([
            ...
            str(args.reference.with_name("replay-fig-introduction-2e8.txt")),
        ...
        if trace.get("input") != {
                ...
                "implicit_quit_calls": 0} or trace.get("boundaries") != {
                    "wait": 1, "poll": 5, "text": 2, "frontend": 0}:
            raise ValueError("FIG 2e8h introduction input boundaries differ")
        if trace.get("video") != {
                "frames": 11, "direct_updates": 4,
                "last_width": 320, "last_height": 200,
                "fnv1a64": expected["rewrite_video_fnv1a64"]}:
            raise ValueError("FIG 2e8h introduction video differs")
        if trace.get("audio") != {
                "music_calls": 1, "voice_calls": 0,
                "stop_music_calls": 0, "stop_audio_calls": 1,
                "fnv1a64": "971fb031ff7f6f85"} or \
                trace.get("delay_milliseconds") != 170:
            raise ValueError("FIG 2e8h introduction audio or timing differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("FIG 2e8h introduction final state differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG 2e8h introduction return boundary differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 11:
            raise ValueError("FIG 2e8h introduction frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    f"FIG 2e8h introduction {page['kind']} no longer matches original")
        ...
        print(
            "FIG 2e8h introduction: both text pages, all four final cursor "
            "phases, the battle command transition, and ten exact original RGB pages")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG 2e8h introduction: FAIL: {error}\n")

#!/usr/bin/env python3
"""Replay and lock FIG's nonfatal player attack followed by a basic monster attack presentation."""
...
EXPECTED_KINDS = (
    ...
    *(f"player_damage_rise{index}" for index in range(1, 11)),
    ...
    "monster_attack_card", "monster_pre_shake_clean",
    *(f"monster_shake_step{index}" for index in range(1, 7)),
    "monster_shake_step8",
    *(f"monster_damage_rise{index}" for index in range(1, 11)),
    "monster_damage_commit", "monster_round_bare_boundary",
    "next_command_page",
)
EXPECTED_REWRITE_FRAMES = (
    *range(1, 11), *range(14, 34), *range(35, 49),
)
...
def digest(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed FIG player-basic digest {label}")
    return value
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != "original_fig_player_attack_basic" or \
                ...
                expected.get("monster_ai_abilities_cleared") is not True or \
                ...
                tuple(page.get("rewrite_frame") for page in pages) != \
                    EXPECTED_REWRITE_FRAMES:
            raise ValueError("unsupported FIG player-basic reference")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from player-basic reference")
        if sha256((args.game / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("FIG player-basic staged game differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("FIG player-basic input evidence differs")
        if expected.get("capture_wait_seconds") != 5 or \
            ...
            raise ValueError("FIG player-basic capture boundary differs")
        ...
        if not isinstance(limitation, str) or \
                ...
                "page 34" not in limitation:
            raise ValueError("FIG player-basic capture limitation missing")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("FIG player-basic replay boundaries differ")
        if trace.get("video") != expected["rewrite_video"] or \
            ...
            raise ValueError("FIG player-basic audio/video timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("FIG player-basic final save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG player-basic replay did not use IF entry")
        ...
        if frame_times != tuple(expected["rewrite_frame_times_milliseconds"]):
            raise ValueError("FIG player-basic frame timing differs")
        voices = tuple((
        ...
        if voices != (
            ...
            raise ValueError("FIG player-basic SP003/SP106 timeline differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != expected["rewrite_video"]["frames"]:
            raise ValueError("FIG player-basic frame count differs")
        previous_original = -1
        for page in pages:
            if page["original_review_frame"] <= previous_original:
                raise ValueError("FIG player-basic original page order differs")
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "FIG player-basic page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")

        print(
            "FIG player-basic checkpoint: physical poses/wipes, ten player "
            "damage pages, bare-background/pose-2 commit, monster attack "
            "card, 2939 shake, ten monster damage pages, commits, boundary "
            "and next command match 44 original RGB pages")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG player-basic checkpoint: FAIL: {error}\n")

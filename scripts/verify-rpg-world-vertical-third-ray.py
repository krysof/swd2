#!/usr/bin/env python3
"""Replay and lock BUIN1 vertical third-ray interaction, facing and restoration."""
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed BUIN1 interaction digest {label}")
...
def mapz_oracle(mapz: bytes) -> tuple[list[int], list[int]]:
    ...
    if header != 512 or location != 4608 or area != 23549 or count != 11:
        raise ValueError("BUIN1 interaction MAPZ structure differs")
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_rpg_buin1_vertical_third_ray_sequence" or \
                ...
                expected.get("player_world_position") != [14, 170] or \
                expected.get("player_direction") != 3 or \
                expected.get("entity_world_footprint") != \
                    [[15, 169], [16, 169], [17, 169]] or \
                expected.get("center_ray_cells") != \
                    [[14, 169], [14, 168], [14, 167], [14, 166]] or \
                expected.get("second_ray_cells") != \
                    [[13, 169], [13, 168], [13, 167], [13, 166]] or \
                expected.get("third_ray_first_cell") != [15, 169] or \
                expected.get("entity_initial_direction") != 6 or \
                expected.get("entity_faced_direction") != 0 or \
                expected.get("entity_restored_direction") != 6 or \
                expected.get("event_directory_offset") != 40 or \
                expected.get("automatic_event_flag") is not False or \
                expected.get("capture_confirm_pulses") != 4 or \
                not isinstance(pages, list) or len(pages) != 5 or \
                [page.get("rewrite_frame") for page in pages] != [0, 2, 3, 4, 5] or \
                [page.get("original_review_frame") for page in pages] != \
                    [238, 463, 501, 505, 575]:
            raise ValueError("unsupported RPG BUIN1 interaction reference")
        ...
        if sha256(release_mapz) != expected["release_mapz_sha256"] or \
            ...
            raise ValueError("release BUIN1 interaction entity differs")
        ...
        if sha256(fixture_mapz) != expected["fixture_mapz_sha256"] or \
            ...
            raise ValueError("staged BUIN1 interaction entity differs")
        ...
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != save_digest or \
            ...
            raise ValueError("RPG BUIN1 interaction save triplets differ")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("RPG BUIN1 interaction input evidence differs")
        if expected.get("capture_wait_seconds") != 6.5 or \
                expected.get("capture_pace_seconds") != 1.5 or \
                expected.get("capture_time_limit_seconds") != 18 or \
                expected.get("capture_review_fps") != 70 or \
                expected.get("capture_video_frames") != 1261 or \
                expected.get("capture_review_frames") != 1259 or \
                expected.get("capture_dosbox_exit_code") != 0 or \
                expected.get("capture_harness_sha256") != HARNESS_SHA256:
            raise ValueError("RPG BUIN1 interaction capture boundary differs")
        for name in (
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("RPG BUIN1 interaction replay timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    expected["rewrite_mapz_fnv1a64"],
                    expected["rewrite_name_fnv1a64"]):
            raise ValueError("RPG BUIN1 interaction final state differs")
        if trace.get("transitions") != [{
                "module": "RPG.EXE", "input": "OC", "output": "--",
                "launched": True}]:
            raise ValueError("RPG BUIN1 interaction replay missed OC entry")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 6:
            raise ValueError("RPG BUIN1 interaction frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "RPG BUIN1 interaction page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")
        print(
            "RPG BUIN1 vertical third ray: vertical third Confirm ray, entity turn during "
            "dialogue and post-event facing restoration match the untouched "
            "original in five complete RGB pages")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"RPG BUIN1 vertical third ray: FAIL: {error}\n")

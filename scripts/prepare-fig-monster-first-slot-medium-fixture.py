#!/usr/bin/env python3
"""Stage a live three-monster battle whose first slot installs AE first."""
...
EXPECTED_ITEM = "86dff2dc87db3c71adf980d744286c48f619fe92d7d78b36216b1125e90b257f"
...
def main() -> int:
    ...
    try:
        ...
        # original's pathological pre-dead-slot state.  Slot 0 has the high
        ...
        set_fields(385, idle)
        set_fields(402, (
            (0x2C, 60000), (0x2E, 9), (0x32, 54), (0x36, 1), (0x38, 0),
        ...
        if save_digest != EXPECTED_SAVE:
            raise ValueError(
                f"FIG monster-first-slot-medium SAVE differs: {save_digest}")
        if item_digest != EXPECTED_ITEM:
            raise ValueError(
                f"FIG monster-first-slot-medium ITEM differs: {item_digest}")
        (args.output / "SAVE.DA1").write_bytes(save)
        item_path.write_bytes(item)
        print(
            "FIG monster-first-slot-medium fixture: "
            f"SAVE={save_digest} ITEM={item_digest}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(1, f"FIG monster-first-slot-medium fixture: FAIL: {error}\n")

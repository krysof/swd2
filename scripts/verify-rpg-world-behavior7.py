#!/usr/bin/env python3
"""Replay and lock MA-DE's original behavior-seven invisible collision."""
...
CROP = [0, 56, 320, 96]
RELEASE_FIELDS = [768, 0, 36114, 7, 10, 0, 65520, 4, 10, 204, 0]
FIXTURE_BEHAVIORS = [3, 3, 3, 3, 3, 3, 3, 3, 3, 7, 3, 3, 3]
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed MA-DE behavior-seven digest {label}")


def u16(data: bytes, offset: int) -> int:
    if offset < 0 or offset + 2 > len(data):
        raise ValueError("MAPZ behavior-seven oracle is out of bounds")
...
def mapz_oracle(mapz: bytes) -> tuple[list[int], list[int]]:
    ...
    if header != 512 or location != 996 or area != 17245 or count != 13:
        raise ValueError("MA-DE MAPZ structure differs")
    field = lambda number, index: u16(
        image, area + 6 + (number * count + index) * 2)
    target = [field(number, 9) for number in range(11)]
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                ...
                tuple(page.get("kind") for page in pages) != (
                    "behavior7_blocker_invisible",
                    "behavior7_collision_blocked") or \
            ...
            raise ValueError("unsupported RPG MA-DE behavior-seven reference")
        ...
        if sha256((args.game / "RPG.EXE").read_bytes()) != \
            ...
            raise ValueError("RPG.EXE differs from behavior-seven reference")
        if sha256(release_mapz) != expected["release_mapz_sha256"] or \
                mapz_oracle(release_mapz)[0] != RELEASE_FIELDS:
            raise ValueError("release MA-DE behavior-seven entity differs")
        ...
        if sha256(fixture_mapz) != expected["fixture_mapz_sha256"] or \
            ...
            raise ValueError("staged MA-DE behavior-seven entity differs")
        if tuple(sha256((args.game / "T1" / name).read_bytes()) for name in
            ...
            raise ValueError("MA-DE resources differ from behavior-seven reference")
        if sha256((args.game / "SA" / "SA003.RSK").read_bytes()) != \
                expected["entity_sprite_archive_sha256"] or \
                sha256((args.game / "RX" / "TO01.RIX").read_bytes()) != \
                    expected["music_sha256"]:
            raise ValueError("MA-DE sprite/music differs from behavior-seven reference")
        ...
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != save_digest or \
            ...
            raise ValueError("RPG MA-DE behavior-seven save triplets differ")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("RPG MA-DE behavior-seven input evidence differs")
        if expected.get("capture_wait_seconds") != 7 or \
                expected.get("capture_pace_seconds") != 0.5 or \
                expected.get("capture_time_limit_seconds") != 10 or \
                expected.get("capture_review_fps") != 70 or \
                expected.get("capture_video_frames") != 700 or \
                expected.get("capture_review_frames") != 699 or \
                expected.get("capture_dosbox_exit_code") != 0 or \
                expected.get("capture_harness_sha256") != HARNESS_SHA256:
            raise ValueError("RPG MA-DE behavior-seven capture boundary differs")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("RPG MA-DE behavior-seven replay timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    expected["rewrite_mapz_fnv1a64"],
                    expected["rewrite_name_fnv1a64"]):
            raise ValueError("RPG MA-DE behavior-seven final state differs")
        if trace.get("transitions") != [{
                "module": "RPG.EXE", "input": "OC", "output": "--",
                "launched": True}]:
            raise ValueError("RPG MA-DE behavior-seven replay missed OC entry")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 2:
            raise ValueError("RPG MA-DE behavior-seven frame count differs")
        for page in pages:
            ...
            if sha256(compared_pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "RPG MA-DE behavior-seven page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")
        print(
            "RPG MA-DE behavior seven: north collision at world (54,101), "
            "no Confirm/text event, before/after 320x96 RGB crops match "
            "the untouched original")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"RPG MA-DE behavior seven: FAIL: {error}\n")

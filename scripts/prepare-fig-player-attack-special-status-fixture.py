#!/usr/bin/env python3
"""Stage a fixed FIG battle whose nonfatal player attack is followed by successful monster special ability 117 status."""
...
def main() -> int:
    ...
    try:
        ...
        if save_digest != EXPECTED_SAVE:
            raise ValueError(f"FIG player-attack-special-status SAVE differs: {save_digest}")
        if item_digest != EXPECTED_ITEM:
            raise ValueError(f"FIG player-attack-special-status ITEM differs: {item_digest}")
        (args.output / "SAVE.DA1").write_bytes(save)
        item_path.write_bytes(item)
        print(
            "FIG player-attack-special-status fixture: "
            f"SAVE={save_digest} ITEM={item_digest}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(1, f"FIG player-attack-special-status fixture: FAIL: {error}\n")

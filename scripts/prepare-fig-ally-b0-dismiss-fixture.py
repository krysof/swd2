#!/usr/bin/env python3
"""Stage captured item 376 after player ability 82 installs B0."""
...
EXPECTED_SAVE = "5b5dd78c995bc0b9494d4cf85b01f7f3da0c00bd2e21bfc98e35eef952b5d759"
...
def main() -> int:
    ...
    try:
        ...
        save[actor + 0x6D] = 82  # selector 3ch installs B0 first
        ...
        if save_digest != EXPECTED_SAVE:
            raise ValueError(
                f"FIG captured-ally B0-dismiss SAVE differs: {save_digest}")
        if item_digest != EXPECTED_ITEM:
            raise ValueError(
                f"FIG captured-ally B0-dismiss ITEM differs: {item_digest}")
        save_path.write_bytes(save)
        item_path.write_bytes(item)
        print(
            "FIG captured-ally B0-dismiss fixture: "
            f"SAVE={save_digest} ITEM={item_digest}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(
            1, f"FIG captured-ally B0-dismiss fixture: FAIL: {error}\n")

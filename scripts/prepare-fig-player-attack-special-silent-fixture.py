#!/usr/bin/env python3
"""Stage a fixed FIG battle whose nonfatal player attack is followed by monster special ability 10's silent return."""
...
EXPECTED_ITEM = "a03a1632df29e5408af1fe9b80715901b7d6aff12b277abe221706c95357d70d"
...
def main() -> int:
    ...
    try:
        ...
        word(item, header_size + record + 0x42, 10)
        word(item, header_size + record + 0x44, 1000)
        word(item, header_size + record + 0x46, 10)
        word(item, header_size + record + 0x4C, 0)

        save_digest = sha256(save)
        item_digest = sha256(item)
        if save_digest != EXPECTED_SAVE:
            raise ValueError(f"FIG player-attack-special-silent SAVE differs: {save_digest}")
        if item_digest != EXPECTED_ITEM:
            raise ValueError(f"FIG player-attack-special-silent ITEM differs: {item_digest}")
        (args.output / "SAVE.DA1").write_bytes(save)
        item_path.write_bytes(item)
        print(
            "FIG player-attack-special-silent fixture: "
            f"SAVE={save_digest} ITEM={item_digest}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(1, f"FIG player-attack-special-silent fixture: FAIL: {error}\n")

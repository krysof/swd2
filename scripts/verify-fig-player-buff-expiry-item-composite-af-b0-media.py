#!/usr/bin/env python3
"""Lock item 209's AF/B0 flights followed by same-turn buff expiry."""
...
def digest(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed composite-af-b0-media-expiry digest {label}")
    return value
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_player_buff_expiry_after_item_composite_af_b0_media" or \
                ...
                (expected.get("item_id"), expected.get("item_type"),
                 ...
                 expected.get("payment_pool")) != \
                    (209, 0x10, 0x6e, 0, 0x6b, 69, 0x8100,
                     [0x3b, 0x3c], [1, 2], "source_actor", 35,
                     "ability_points") or \
                expected.get("expiry_player_turn") != 4 or \
                not isinstance(pages, list) or len(pages) != 79 or \
                ...
                tuple(page.get("rewrite_frame") for page in rewrite_only) != \
                    REWRITE_ONLY_FRAMES:
            raise ValueError("unsupported FIG composite-af-b0-media-expiry reference")

        original = (args.game / "FIG.EXE").read_bytes()
        if sha256(original) != expected["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from composite-af-b0-media-expiry reference")
        ...
        for offset_key, code_key, offset in checks:
            ...
            if expected.get(offset_key) != offset or not isinstance(code, str) or \
                ...
                raise ValueError(
                    "FIG composite-af-b0-media-expiry instruction differs: " + code_key)
        item_archive = (args.game / "ITEM.EXE").read_bytes()
        if sha256(item_archive) != expected["item_archive_sha256"]:
            raise ValueError("ITEM.EXE differs from composite-af-b0-media-expiry reference")
        item = archive_record(item_archive, 209 + 2)
        if len(item) < 11 or \
                (int.from_bytes(item[0:2], "little"), item[5], item[6],
                 int.from_bytes(item[7:9], "little"), item[9], item[10]) != \
                    (0x10, 0x6e, 0, 0x6b, 0x3b, 0x3c):
            raise ValueError("ITEM.EXE item 209 composite record differs")
        if sha256((args.game / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("FIG composite-af-b0-media-expiry fixture differs")
        ...
        if sha256(autotype.read_bytes()) != expected["capture_autotype_sha256"] or \
            ...
            raise ValueError("FIG composite-af-b0-media-expiry inputs differ")
        if (expected.get("capture_wait_seconds"),
            ...
            raise ValueError("FIG composite-af-b0-media-expiry capture boundary differs")
        for name in ("capture_video_sha256", "capture_manifest_sha256"):
            digest(expected.get(name), name)
        limitation = expected.get("capture_limitation")
        if not isinstance(limitation, str) or \
                "Seventy-nine stable full 320x200 RGB pages" not in limitation or \
                ...
                "3Ch installs B0" not in limitation or \
                "35-point" not in limitation or \
                "deliberately unpaired" not in limitation or \
                not all(token in limitation for token in (
                    "0a84", "11fc", "1235", "57f2", "4792", "5b41",
                    "0c41", "0da7", "0d98")):
            raise ValueError("FIG composite-af-b0-media-expiry limitation is missing")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("FIG composite-af-b0-media-expiry rewrite trace differs")
        if trace.get("stop_reason") != "module requested exit" or \
                trace.get("final_marker") != "--" or trace.get("transitions") != [{
                    "module": "FIG.EXE", "input": "IF", "output": "--",
                    "launched": True}]:
            raise ValueError("FIG composite-af-b0-media-expiry missed replay exit")
        ...
        timed = ((120, "item_pose_at_milliseconds"),
                 ...
                 (177, "last_second_flight_at_milliseconds"),
                 (178, "paid_dark_at_milliseconds"),
                 (183, "restored_two_media_at_milliseconds"),
                 (184, "expiry_at_milliseconds"), (185, "clean_at_milliseconds"),
                 (186, "monster_attack_at_milliseconds"),
                 (206, "monster_tail_at_milliseconds"),
                 (207, "round_boundary_at_milliseconds"),
                 (208, "next_command_at_milliseconds"))
        if any(times[index] != expected[key] for index, key in timed) or \
                times[184] - times[183] != 55 or \
                times[185] - times[184] != expected["expiry_hold_milliseconds"] or \
                times[186] - times[185] != expected["clean_hold_milliseconds"]:
            raise ValueError("FIG composite-af-b0-media-expiry timing differs")
        ...
        if len(voices) != 10 or \
            ...
            raise ValueError("FIG composite-af-b0-media-expiry voices differ")

        frames = load_indexed_frames(frame_path)
        if len(frames) != expected["rewrite_video"]["frames"]:
            raise ValueError("FIG composite-af-b0-media-expiry frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError("FIG composite-af-b0-media-expiry exact page differs: " + page["kind"])
            ...
            if not isinstance(page.get("original_review_frame"), int) or \
                ...
                raise ValueError(
                    "FIG composite-af-b0-media-expiry original range differs: " +
                    page["kind"])
        for page in cropped:
            pixels, palette = frames[page["rewrite_frame"]]
            rgb = expand_rgb(pixels, palette)
            ...
            if page.get("crop") != [0, 0, 320, 197] or \
                ...
                raise ValueError(
                    "FIG composite-af-b0-media-expiry crop differs: " +
                    page["kind"])
        ...
        for page in rewrite_only:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError("FIG composite-af-b0-media-expiry rewrite-only page differs: " + page["kind"])
        if frames[183] == frames[184] or frames[184] == frames[185]:
            raise ValueError("FIG composite-af-b0-media-expiry retained-page ownership differs")

        print(
            "FIG composite-af-b0-media expiry checkpoint: 79 exact RGB "
            "pages and five exact top-197 crops lock AF-before-B0 flights, "
            "retained media, same-turn expiry and following cleanup")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG composite-af-b0-media expiry checkpoint: FAIL: {error}\n")

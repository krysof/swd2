#!/usr/bin/env python3
"""Replay and lock FIG's target-anchored, faded 59a1 immunity sequence."""
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed monster-immunity digest {label}")
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != "original_fig_monster_immunity" or \
                expected.get("formation_directory_offset") != 392 or \
                (expected.get("ability_id"), expected.get("effect_code")) != (6, 0x5E):
            raise ValueError("unsupported FIG monster-immunity reference")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from the immunity reference")
        autotype = args.reference.with_name(expected["capture_autotype"])
        if sha256(autotype.read_bytes()) != expected["capture_autotype_sha256"]:
            raise ValueError("original immunity capture input differs")
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
                expected["fixture_save_sha256"]:
            raise ValueError("FIG monster-immunity staged save differs")
        ...
        subprocess.run([
            ...
            str(args.reference.with_name("replay-fig-monster-immunity.txt")),
        ...
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
            ...
            raise ValueError("FIG monster-immunity state differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG monster-immunity did not use IF entry")
        ...
        for page in matched:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    f"FIG monster-immunity {page['kind']} differs from original")
            digest(page.get("original_png_sha256"),
                   f"{page.get('kind')}/original_png_sha256")
            if not isinstance(page.get("original_review_frame"), int):
                raise ValueError("FIG monster-immunity review frame differs")
        ...
        parser.exit(1, f"FIG monster-immunity checkpoint: FAIL: {error}\n")

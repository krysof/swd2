#!/usr/bin/env python3
"""Replay and lock MA-DE's original behavior-six collision removal."""
...
CROP = [80, 0, 160, 120]
RELEASE_FIELDS = [768, 0, 51342, 6, 15, 0, 65520, 4, 10, 20, 0]
FIXTURE_BEHAVIORS = [3, 3, 3, 3, 3, 6, 3, 3, 3, 3, 3, 3, 3]
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed MA-DE behavior-six digest {label}")


def u16(data: bytes, offset: int) -> int:
    if offset < 0 or offset + 2 > len(data):
        raise ValueError("MAPZ behavior-six oracle is out of bounds")
...
def mapz_oracle(mapz: bytes) -> tuple[list[int], list[int]]:
    ...
    target = [field(number, 5) for number in range(11)]
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_rpg_ma_de_behavior6_collision_sequence" or \
                ...
                tuple(page.get("kind") for page in pages) != (
                    "behavior6_entity_visible", "behavior6_entity_removed") or \
            ...
            raise ValueError("unsupported RPG MA-DE behavior-six reference")
        ...
        if sha256((args.game / "RPG.EXE").read_bytes()) != \
            ...
            raise ValueError("RPG.EXE differs from behavior-six reference")
        if sha256(release_mapz) != expected["release_mapz_sha256"] or \
                mapz_oracle(release_mapz)[0] != RELEASE_FIELDS:
            raise ValueError("release MA-DE behavior-six entity differs")
        ...
        if sha256(fixture_mapz) != expected["fixture_mapz_sha256"] or \
            ...
            raise ValueError("staged MA-DE behavior-six entity differs")
        if tuple(sha256((args.game / "T1" / name).read_bytes()) for name in
            ...
            raise ValueError("MA-DE resources differ from behavior-six reference")
        if sha256((args.game / "SA" / "SA003.RSK").read_bytes()) != \
            ...
            raise ValueError("MA-DE sprite/music differs from behavior-six reference")
        ...
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != save_digest or \
            ...
            raise ValueError("RPG MA-DE behavior-six save triplets differ")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("RPG MA-DE behavior-six input evidence differs")
        if expected.get("capture_wait_seconds") != 4.7 or \
            ...
            raise ValueError("RPG MA-DE behavior-six capture boundary differs")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("RPG MA-DE behavior-six replay timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    expected["rewrite_mapz_fnv1a64"],
                    expected["rewrite_name_fnv1a64"]):
            raise ValueError("RPG MA-DE behavior-six final state differs")
        if trace.get("transitions") != [{
                "module": "RPG.EXE", "input": "OC", "output": "--",
                "launched": True}]:
            raise ValueError("RPG MA-DE behavior-six replay missed OC entry")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 2:
            raise ValueError("RPG MA-DE behavior-six frame count differs")
        for page in pages:
            ...
            if sha256(compared_pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "RPG MA-DE behavior-six page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")
        print(
            "RPG MA-DE behavior six: east collision at world (106,142), "
            "no Confirm/text event, visible/removed 160x120 RGB crops match "
            "the untouched original")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"RPG MA-DE behavior six: FAIL: {error}\n")

#!/usr/bin/env python3
"""Stage a fixed FIG battle whose nonfatal player attack is followed by monster special ability 38's self buff."""
...
EXPECTED_ITEM = "29fb59bf9d274417cf5165904adb3ade4207b1321807da6213f1506c6fbf68d3"
...
def main() -> int:
    ...
    try:
        ...
        word(item, header_size + record + 0x42, 38)
        word(item, header_size + record + 0x44, 1000)
        word(item, header_size + record + 0x46, 38)
        word(item, header_size + record + 0x4C, 0)

        save_digest = sha256(save)
        item_digest = sha256(item)
        if save_digest != EXPECTED_SAVE:
            raise ValueError(f"FIG player-attack-special-buff SAVE differs: {save_digest}")
        if item_digest != EXPECTED_ITEM:
            raise ValueError(f"FIG player-attack-special-buff ITEM differs: {item_digest}")
        (args.output / "SAVE.DA1").write_bytes(save)
        item_path.write_bytes(item)
        print(
            "FIG player-attack-special-buff fixture: "
            f"SAVE={save_digest} ITEM={item_digest}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(1, f"FIG player-attack-special-buff fixture: FAIL: {error}\n")

...
EXPECTED_ORIGINAL = (
    3908, 3917, 3921, 3925, 3929, 4008, 4078, 4101, 4119, 4121,
    4123, 4125, 4127, 4129, 4131, 4135, 4137, 4139, 4141, 4143,
    4145, 4147, 4149, 4151, 4155, 4157, 4188, 4210,
)
EXPECTED_STABLE = (
    3915, 3919, 3923, 3927, 3966, 4077, 4100, 4118, 4120, 4122,
    4124, 4126, 4128, 4130, 4134, 4136, 4138, 4140, 4142, 4144,
    4146, 4148, 4150, 4154, 4156, 4187, 4208, 4258,
)
REWRITE_ONLY_KINDS = (
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_player_buff_expiry_after_empty_b0_medium_item" or \
                ...
                (expected.get("item_id"), expected.get("item_effect_code"),
                 expected.get("embedded_ability_id")) != (223, 0x3f, 83) or \
        ...
        if not isinstance(limitation, str) or \
                ...
                "Repeated final right/confirm" not in limitation or \
                not all(token in limitation for token in (
                    "0a84", "11fc", "1235", "4894", "482e", "0c41", "0da7",
                    "0d98")):

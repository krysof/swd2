#!/usr/bin/env python3
"""Stage captured item 376 after player ability 51 installs AE."""
...
EXPECTED_SAVE = "d69b1b624ed783680b2aecd6a5ce84fdefa1a0dbbe42170202bc9dd057d236f7"
EXPECTED_ITEM = "807353c9bf048ced22ec0c6745e5a4774bb7529b24540bd3d37991c1c783d876"
...
def main() -> int:
    ...
    try:
        ...
        save[actor + 0x6D] = 51  # selector 31h installs AE first
        ...
        # special-B ability 53/effect 3dh without changing that effect record.
        for offset, value in (
        ...
        if save_digest != EXPECTED_SAVE:
            raise ValueError(
                f"FIG captured-ally AE-dismiss SAVE differs: {save_digest}")
        if item_digest != EXPECTED_ITEM:
            raise ValueError(
                f"FIG captured-ally AE-dismiss ITEM differs: {item_digest}")
        save_path.write_bytes(save)
        item_path.write_bytes(item)
        print(
            "FIG captured-ally AE-dismiss fixture: "
            f"SAVE={save_digest} ITEM={item_digest}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(
            1, f"FIG captured-ally AE-dismiss fixture: FAIL: {error}\n")

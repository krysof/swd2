#!/usr/bin/env python3
"""Stage captured item 376 with a deterministic special-B AE dismissal."""
...
def main() -> int:
    ...
    try:
        ...
        if save_digest != EXPECTED_SAVE:
            raise ValueError(
                f"FIG captured-ally empty-AE SAVE differs: {save_digest}")
        if item_digest != EXPECTED_ITEM:
            raise ValueError(
                f"FIG captured-ally empty-AE ITEM differs: {item_digest}")
        save_path.write_bytes(save)
        item_path.write_bytes(item)
        print(
            "FIG captured-ally empty-AE fixture: "
            f"SAVE={save_digest} ITEM={item_digest}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(
            1, f"FIG captured-ally empty-AE fixture: FAIL: {error}\n")

#!/usr/bin/env python3
"""Replay and lock all four actor-specific initial RPG Magic pages."""
...
def main() -> int:
    ...
    try:
        ...
        if reference.get("schema_version") != 1 or len(actors) != 4:
            raise ValueError("unsupported Magic-actor reference")
        args.output.mkdir(parents=True, exist_ok=True)
        for actor, expected in enumerate(actors):
            if expected.get("actor") != actor:
                raise ValueError("Magic actors are not in identity order")
            replay = args.reference.parent / f"replay-rpg-magic-actor-{actor}.txt"
            ...
            if hashlib.sha256(pixels).hexdigest() != \
                ...
                raise ValueError(f"actor {actor} indexed Magic page differs")
            if hashlib.sha256(palette).hexdigest() != \
                    expected["rewrite_palette_sha256"]:
                raise ValueError(f"actor {actor} Magic palette differs")
        ...
        print(
            "RPG Magic actors checkpoint: four complete indexed/palette "
            "pages and four original 64,000-pixel RGB pages match")
        return 0
    except (OSError, ValueError, KeyError, json.JSONDecodeError,
            subprocess.SubprocessError) as error:
        parser.exit(1, f"RPG Magic actors checkpoint: FAIL: {error}\n")

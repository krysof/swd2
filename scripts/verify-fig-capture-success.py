#!/usr/bin/env python3
"""Replay and lock FIG's random-encounter capture-success sequence."""
...
PAGE_FRAMES = tuple(range(1, 10))
PAGE_LABELS = (
    "actor-zero initial command page",
    "main command cursor moved to tactics",
    "random-encounter tactics page",
    "capture tile selected",
    "0ded target-anchored base fighter pose",
    "0da7 target-anchored capture card",
    "0e84 captured monster removed after eighteen ticks",
    "capture success clean page after nine ticks",
    "victory page after capture removed the only monster",
)
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed FIG capture-success digest {label}")
...
def main() -> int:
    ...
    try:
        ...
        if reference.get("schema_version") != 1 or \
                reference.get("kind") != \
                    "original_fig_random_capture_success_sequence" or \
            ...
            raise ValueError("unsupported FIG capture-success reference")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                reference["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from capture-success reference")
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
                reference["fixture_save_sha256"]:
            raise ValueError("FIG capture-success staged SAVE differs")
        if sha256((args.game / "ORC.EXE").read_bytes()) != \
                reference["fixture_orc_sha256"]:
            raise ValueError("FIG capture-success staged ORC differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("FIG capture-success original AUTOTYPE differs")
        if sha256(replay.read_bytes()) != reference["replay_input_sha256"]:
            raise ValueError("FIG capture-success replay input differs")
        if reference.get("capture_wait_seconds") != 5 or \
            ...
            raise ValueError("FIG capture-success capture boundary differs")
        ...
        if trace.get("input") != {
                ...
                "implicit_quit_calls": 0} or trace.get("boundaries") != {
                    "wait": 5, "poll": 0, "text": 0, "frontend": 86}:
            raise ValueError("FIG capture-success replay boundaries differ")
        if trace.get("video") != reference["rewrite_video"] or \
            ...
            raise ValueError("FIG capture-success timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    reference["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("FIG capture-success final save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG capture-success replay missed IF entry")
        ...
        if tuple(frame_times.get(index) for index in PAGE_FRAMES) != \
                (0, 0, 0, 0, 0, 0, 989, 1483, 1593):
            raise ValueError("FIG capture-success frame timing differs")
        ...
        if not isinstance(checks, list) or len(checks) != 2 or len(voices) != 2:
            raise ValueError("FIG capture-success voice count differs")
        for voice, expected in zip(voices, checks):
            if (voice.get("call"), voice.get("at_milliseconds"),
                    ...
                    expected.get("resource") not in ("SP016.VOC", "SP015.VOC"):
                raise ValueError("FIG capture-success voice checkpoint differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != reference["rewrite_video"]["frames"]:
            raise ValueError("FIG capture-success frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "FIG capture-success page differs from original: " +
                    page["label"])
        ...
        print(
            "FIG capture-success checkpoint: command pages, target anchor, "
            "SP016/SP015, 18/9-tick holds and victory match original RGB")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG capture-success checkpoint: FAIL: {error}\n")

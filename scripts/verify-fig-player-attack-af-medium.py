#!/usr/bin/env python3
"""Replay and lock a physical hit followed by FIG's generic monster AF mediator install."""
...
EXPECTED_FRAMES = (
    *range(1, 11), *range(14, 28), *range(29, 58),
)
EXPECTED_KINDS = (
    ...
    *(f"player_damage_rise{index}" for index in range(1, 11)),
    ...
    "monster_medium_prepare_bare", "fixed_af_medium_name_card",
    *(f"medium_flight_step_{index:02d}" for index in range(2, 28)),
...
def digest(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed player-attack-af-medium digest {label}")
    return value
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_player_attack_af_medium_install" or \
            ...
            raise ValueError("unsupported FIG player-attack-af-medium reference")

        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from player-attack-af-medium reference")
        if sha256((args.game / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("player-attack-af-medium staged game differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("player-attack-af-medium input evidence differs")
        if expected.get("capture_wait_seconds") != 5 or \
            ...
            raise ValueError("player-attack-af-medium capture boundary differs")
        ...
        if not isinstance(limitation, str) or \
            ...
            raise ValueError("player-attack-af-medium capture limitation missing")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("player-attack-af-medium replay boundaries differ")
        if trace.get("video") != expected["rewrite_video"] or \
            ...
            raise ValueError("player-attack-af-medium audio/video timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("player-attack-af-medium final save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("player-attack-af-medium replay missed IF entry")
        ...
        if frame_times != tuple(expected["rewrite_frame_times_milliseconds"]):
            raise ValueError("player-attack-af-medium frame timing differs")
        ...
        if voices != (
            ...
            raise ValueError("player-attack-af-medium voice timeline differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != expected["rewrite_video"]["frames"]:
            raise ValueError("player-attack-af-medium frame count differs")
        previous_original = -1
        for page in pages:
            review = page.get("original_review_frame")
            if not isinstance(review, int) or review <= previous_original:
                raise ValueError("player-attack-af-medium original order differs")
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "player-attack-af-medium page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")

        print(
            "FIG player-attack-af-medium checkpoint: physical tail, fixed AF "
            "name card, 26 sampled mediator-flight positions, installed-medium "
            "cleanup and next command match 53 original RGB pages")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG player-attack-af-medium checkpoint: FAIL: {error}\n")

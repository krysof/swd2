#!/usr/bin/env python3
"""Replay and lock MA-DE's released behavior-two animation cycle."""
...
CROP = [240, 40, 64, 80]
RELEASE_FIELDS = [11264, 0, 31894, 2, 15, 0, 65520, 4, 10, 144, 0]
FIXTURE_BEHAVIORS = [3, 3, 3, 3, 2, 3, 3, 3, 3, 3, 3, 3, 3]
TARGET_STATES = [
    {"world": [103, 88], "direction": 0, "animation": 1,
     "rewrite_frame": 1, "original_review_frame": 413},
    {"world": [103, 88], "direction": 0, "animation": 2,
     "rewrite_frame": 17, "original_review_frame": 536},
    {"world": [103, 88], "direction": 0, "animation": 3,
     "rewrite_frame": 33, "original_review_frame": 659},
    {"world": [103, 88], "direction": 0, "animation": 0,
...
def mapz_oracle(mapz: bytes) -> tuple[list[int], list[int]]:
    ...
    return ([field(number, 4) for number in range(11)],
...
def main() -> int:
    ...
    try:
        ...
        if sha256(release_mapz) != expected["release_mapz_sha256"] or \
            ...
            raise ValueError("release MA-DE autonomous animation differs")
        ...
        if sha256(fixture_mapz) != expected["fixture_mapz_sha256"] or \
            ...
            raise ValueError("staged MA-DE autonomous animation differs")
        ...
        if sha256((args.game / "SA" / "SA044.RSK").read_bytes()) != \
        ...
        for page in pages:
            ...
            if sha256(compared_pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "RPG MA-DE animation page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")
        print(
            "RPG MA-DE autonomous animation: 60 idle polls, released "
            "behavior-two frames 1/2/3/0 and complete 64x80 actor crops "
            "match the untouched original")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"RPG MA-DE autonomous animation: FAIL: {error}\n")

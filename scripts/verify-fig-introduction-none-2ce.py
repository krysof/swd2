...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != "original_fig_introduction_without_prompt_2ce" or \
                expected.get("formation_directory_offset") != 0x2CE or \
        ...
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("FIG 2ceh introduction staged save differs")
        ...
        if trace.get("input") != {
            ...
            raise ValueError("FIG 2ceh introduction input boundaries differ")
        if trace.get("video") != {
                "frames": 8, "direct_updates": 2,
                "last_width": 320, "last_height": 200,
                "fnv1a64": expected["rewrite_video_fnv1a64"]}:
            raise ValueError("FIG 2ceh introduction video differs")
        if trace.get("audio") != {
            ...
            raise ValueError("FIG 2ceh introduction audio or timing differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("FIG 2ceh introduction final state differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG 2ceh introduction return boundary differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 8:
            raise ValueError("FIG 2ceh introduction frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    f"FIG 2ceh introduction {page['kind']} no longer matches original")
        ...
        print(
            "FIG 2ceh no-prompt introduction: bare enemy page, first glyph, all four "
            "animated cursor frames, command page, and seven exact original "
            "RGB pages")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG 2ceh no-prompt introduction: FAIL: {error}\n")

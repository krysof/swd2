#!/usr/bin/env python3
"""Stage AREA1 for the original four-cell north viewport scroll."""
...
EXPECTED_SAVE_SHA256 = "6e6554cb35983acbddbd894074792eea895ab3e87b2c7a494ecc3db22bde7830"
...
def main() -> int:
    ...
    try:
        ...
        set_word(save, 0x41B, 106)
        set_word(save, 0x41D, 5)
        set_word(save, 0x40D, 2020)
        actual = hashlib.sha256(save).hexdigest()
        if actual != EXPECTED_SAVE_SHA256:
            raise ValueError(f"north-scroll SAVE differs: {actual}")
        for name in ("SAVE.DA1", "SAVE.DAQ"): (args.output / name).write_bytes(save)
        for source, destinations in (("MAPZ.DA1", ("MAPZ.DA1", "MAPZ.DAQ")), ("NAME1.DSK", ("NAME1.DSK", "NAMEQ.DSK"))):
            for destination in destinations: shutil.copy2(args.game / source, args.output / destination)
        print(f"RPG AREA1 north-scroll fixture: {actual}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(1, f"RPG AREA1 north-scroll fixture: FAIL: {error}\n")

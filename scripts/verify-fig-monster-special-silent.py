#!/usr/bin/env python3
"""Replay and lock FIG 26af's unhandled-selector silent return."""
...
PAGE_FRAMES = (3, 4, 5, 6, 7, 26)
PAGE_LABELS = (
    "target selection immediately before the silent action",
    "20e7 bare preparation with no ability-name card",
    "22e0 bare cleanup with no ability-name card",
    "following monster physical-attack card",
    "following monster attack first clean page",
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed monster-special-silent digest {label}")
...
def main() -> int:
    ...
    try:
        ...
        if reference.get("schema_version") != 1 or \
                reference.get("kind") != \
                    "original_fig_monster_special_silent_return_sequence" or \
            ...
            raise ValueError("unsupported FIG monster-special-silent reference")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                reference["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from silent-return reference")
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
                reference["fixture_save_sha256"]:
            raise ValueError("monster-special-silent staged SAVE differs")
        autotype = args.reference.with_name(reference["capture_autotype"])
        if sha256(autotype.read_bytes()) != \
                reference["capture_autotype_sha256"]:
            raise ValueError("monster-special-silent original AUTOTYPE differs")
        if reference.get("capture_wait_seconds") != 5 or \
            ...
            raise ValueError("monster-special-silent capture boundary differs")
        ...
        replay = args.reference.with_name(
            "replay-fig-monster-special-silent.txt")
        subprocess.run([
        ...
        if trace.get("video") != reference["rewrite_video"] or \
            ...
            raise ValueError("monster-special-silent timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    reference["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("monster-special-silent final save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("monster-special-silent replay missed IF entry")
        ...
        if len(frames) != reference["rewrite_video"]["frames"]:
            raise ValueError("monster-special-silent frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "monster-special-silent page differs from original: "
                    f"{page['label']}")
        ...
        print(
            "FIG monster-special-silent checkpoint: 26af effect 4c exposes "
            "only bare 20e7/22e0 pages; six pages exactly match original RGB")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG monster-special-silent checkpoint: FAIL: {error}\n")

#!/usr/bin/env python3
"""Stage MA-DE's released behavior-two autonomous animator in isolation."""
...
EXPECTED_SAVE_SHA256 = \
    "b128371b907c0da86e448352ab191d8fa566780fde69e9c6db42551136e86b77"
EXPECTED_MAPZ_SHA256 = \
    "8bfe117a6beeb09b0690ed2e10b2de7e2b10f49e9958b46ee3360969311ec651"
RELEASE_BEHAVIORS = (1, 3, 0, 2, 2, 6, 6, 3, 6, 7, 7, 7, 7)
RELEASE_TARGET_FIELDS = (11264, 0, 31894, 2, 15, 0, 65520, 4, 10, 144, 0)
...
def main() -> int:
    ...
    try:
        ...
        target = tuple(u16(image, area + 6 + (field * count + 4) * 2)
        ...
        if header != 512 or location != 996 or area != 17245 or count != 13 or \
            ...
            raise ValueError("release MA-DE autonomous animation boundary changed")
        # Preserve entity four byte-for-byte.  Hide the other twelve actors in
        ...
        if actual_mapz != EXPECTED_MAPZ_SHA256:
            raise ValueError(f"autonomous-animation MAPZ differs: {actual_mapz}")
        ...
        # Entity four is fixed at world (103,88).  Viewport (70,78) keeps
        # its complete four-frame animation in the evidence crop while the
        # stationary party remains at world (90,90).
        set_word(save, 0x408, 0x2010)
        set_word(save, 0x40d, 28228)
        set_word(save, 0x40f, 8)
        set_word(save, 0x424, 10)
        set_word(save, 0x012, 38)
        set_word(save, 0x02a, 80)
        set_word(save, 0x0a2, 0)
        set_word(save, 0x41b, 70)
        set_word(save, 0x41d, 78)
        ...
        if actual_save != EXPECTED_SAVE_SHA256:
            raise ValueError(f"autonomous-animation SAVE differs: {actual_save}")
        ...
        print(
            f"RPG MA-DE autonomous-animation fixture: SAVE {actual_save}, "
            f"MAPZ {actual_mapz}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(1, f"RPG MA-DE autonomous-animation fixture: FAIL: {error}\n")

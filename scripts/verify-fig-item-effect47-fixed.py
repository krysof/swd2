#!/usr/bin/env python3
"""Replay fixed-battle item 237 and lock selector 47h's rejection unwind."""
...
def main() -> int:
    ...
    try:
        ...
        if reference.get("schema_version") != 1 or \
                reference.get("kind") != "original_fig_item_effect47_fixed" or \
                ...
                tuple(page.get("rewrite_frame") for page in pages) != \
                    EXPECTED_REWRITE_FRAMES:
            raise ValueError("unsupported fixed FIG item-effect-47 reference")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                reference["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from fixed item-effect-47 reference")
        if sha256((args.game / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("fixed FIG item-effect-47 staged game differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("fixed FIG item-effect-47 input evidence differs")
        if reference.get("capture_wait_seconds") != 5 or \
            ...
            raise ValueError("fixed FIG item-effect-47 capture boundary differs")
        ...
        if trace.get("input") != reference["rewrite_input"] or \
            ...
            raise ValueError("fixed FIG item-effect-47 replay boundaries differ")
        if trace.get("video") != reference["rewrite_video"] or \
            ...
            raise ValueError("fixed FIG item-effect-47 timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    reference["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("fixed FIG item-effect-47 final save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("fixed FIG item-effect-47 did not stay in battle")
        ...
        if frame_times != tuple(reference["rewrite_frame_times_milliseconds"]):
            raise ValueError("fixed FIG item-effect-47 frame timing differs")
        ...
        if actual_voices != expected_voices:
            raise ValueError("fixed FIG item-effect-47 voice timeline differs")
        if any(item[2] == "94bb5914d0c4e7cf" for item in actual_voices):
            raise ValueError("fixed FIG item-effect-47 incorrectly used SV3")

        frames = load_indexed_frames(frame_path)
        if len(frames) != reference["rewrite_video"]["frames"]:
            raise ValueError("fixed FIG item-effect-47 frame count differs")
        previous_original = -1
        for page in pages:
            if page["original_review_frame"] <= previous_original:
                raise ValueError("fixed FIG item-effect-47 original page order differs")
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "fixed FIG item-effect-47 page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")

        print(
            "Fixed FIG item effect 47 checkpoint: item/target pages, pose0, "
            "SP071/five-tick unwind, immediate monster turn and next command "
            "match 27 original RGB pages without resource debit, item consumption, SV3 or a "
            "normal escape-failure card"
        )
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"Fixed FIG item effect 47 checkpoint: FAIL: {error}\n")

...
def main() -> int:
    ...
    try:
        ...
        if video.get("frames") != 115 or \
                video.get("last_width") != 320 or \
                video.get("last_height") != 200 or \
                video.get("fnv1a64") != "0f2614ed2a99df2f":
            raise ValueError("System replay video summary differs")
        hashes = data.get("frame_fnv1a64", [])
        if len(hashes) != 115 or hashes[114] != "2f11d877c82ae183":
            raise ValueError("default RPG System page differs")
        ...
        print("RPG System menu checkpoint: 115 frames, default page locked")
        return 0

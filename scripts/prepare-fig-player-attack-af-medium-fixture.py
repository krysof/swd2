#!/usr/bin/env python3
"""Stage a fixed FIG battle whose monster ability 8 installs missing AF."""
...
EXPECTED_ITEM = "293ac2320b465f00ac1ab10f3f45f1cafc286df2c9ac53d4d4ecb8f2047019f4"
...
def main() -> int:
    ...
    try:
        ...
        # Ability 8 has target_flags 8140h.  The monster 23b1 path installs
        # the absent AF mediator before attempting its ordinary effect 45h.
        word(item, header_size + record + 0x32, 8)
        ...
        if save_digest != EXPECTED_SAVE:
            raise ValueError(
                f"FIG player-attack-AF-medium SAVE differs: {save_digest}")
        if item_digest != EXPECTED_ITEM:
            raise ValueError(
                f"FIG player-attack-AF-medium ITEM differs: {item_digest}")
        (args.output / "SAVE.DA1").write_bytes(save)
        item_path.write_bytes(item)
        print(
            "FIG player-attack-AF-medium fixture: "
            f"SAVE={save_digest} ITEM={item_digest}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(
            1, f"FIG player-attack-AF-medium fixture: FAIL: {error}\n")

#!/usr/bin/env python3
"""Run the two-document IDBFS probe in the installed branded Firefox app."""
...
def main() -> int:
    ...
    try:
        ...
        info = firefox / "Contents" / "Info.plist"
        if not info.is_file():
            raise ValueError("branded Firefox.app is absent")
        ...
        if bundle_id != "org.mozilla.firefox" or not version or not build:
            raise ValueError("Firefox bundle identity/version differs")
        ...
        try:
            for cycle in range(args.cycles):
                ...
                token = f"firefox-brand-roundtrip-{cycle}-{secrets.token_hex(8)}"
                query = urllib.parse.urlencode({
                    "idbfs-self-test": token,
                    "idbfs-report": nonce,
                })
                url = f"http://127.0.0.1:{server.server_port}/?{query}"
                subprocess.run(["open", "-a", str(firefox), url], check=True)
                ...
                while datetime.datetime.now().timestamp() < deadline:
                    ...
                    if candidate.get("nonce") == nonce:
                        ...
                        break
                if matched is None:
                    raise ValueError(f"Firefox cycle {cycle} did not report")
                ...
                if matched.get("passed") is not True or \
                        ...
                        "Firefox/" not in user_agent or "Safari/" in user_agent or \
                        "Chrome/" in user_agent or "Chromium/" in user_agent:
                    raise ValueError(
                        f"Firefox cycle {cycle} result differs: {matched!r}")
                results.append({
                    "cycle": cycle,
                    "status": "verified",
        ...
        report = {
            "schema_version": 1,
            "kind": "wasm_branded_firefox_idbfs_restart",
            "status": "verified",
            "limitation": (
                "Installed macOS Firefox is not physical Android/Firefox and this "
        ...
        print(
            f"WASM branded Firefox IDBFS: OK ({args.cycles} two-document "
            f"restart cycle{'s' if args.cycles != 1 else ''}, Firefox {version})")
        return 0
    except (OSError, ValueError, subprocess.SubprocessError) as error:
        parser.exit(1, f"WASM branded Firefox IDBFS: FAIL: {error}\n")

#!/usr/bin/env python3
"""Replay and lock AREA1's original behavior-three hidden traversal."""
...
CROP = [0, 0, 320, 200]
RELEASE_FIELDS = [0, 0, 9270, 3, 10, 1, 65532, 4, 10, 40, 0]
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed AREA1 behavior-three digest {label}")


def u16(data: bytes, offset: int) -> int:
    if offset < 0 or offset + 2 > len(data):
        raise ValueError("MAPZ behavior-three oracle is out of bounds")
...
def mapz_oracle(mapz: bytes) -> tuple[list[int], list[int]]:
    ...
    target = [field(number, 0) for number in range(11)]
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_rpg_area1_behavior3_traversal_sequence" or \
                ...
                expected.get("initial_world_position") != [129, 25] or \
                expected.get("requested_direction") != "RIGHT" or \
                expected.get("confirm_inputs") != 0 or \
                expected.get("text_boundaries") != 0 or \
                expected.get("final_world_position") != [130, 25] or \
                expected.get("traversed_hidden_cell") != [131, 25] or \
                ...
                tuple(page.get("kind") for page in pages) != (
                    "behavior3_hidden_initial",
                    "behavior3_footprint_traversed") or \
            ...
            raise ValueError("unsupported RPG AREA1 behavior-three reference")
        ...
        if sha256((args.game / "RPG.EXE").read_bytes()) != \
            ...
            raise ValueError("RPG.EXE differs from behavior-three reference")
        if sha256(release_mapz) != expected["release_mapz_sha256"] or \
                mapz_oracle(release_mapz)[0] != RELEASE_FIELDS:
            raise ValueError("release AREA1 behavior-three entity differs")
        ...
        if sha256(fixture_mapz) != expected["fixture_mapz_sha256"] or \
            ...
            raise ValueError("staged AREA1 behavior-three entity differs")
        if tuple(sha256((args.game / "T1" / name).read_bytes()) for name in
            ...
            raise ValueError("AREA1 resources differ from behavior-three reference")
        if sha256((args.game / "SA" / "SA068.RSK").read_bytes()) != \
            ...
            raise ValueError("AREA1 sprite/music differs from behavior-three reference")
        ...
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != save_digest or \
            ...
            raise ValueError("RPG AREA1 behavior-three save triplets differ")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("RPG AREA1 behavior-three input evidence differs")
        if expected.get("capture_wait_seconds") != 8.5 or \
            ...
            raise ValueError("RPG AREA1 behavior-three capture boundary differs")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("RPG AREA1 behavior-three replay timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    expected["rewrite_mapz_fnv1a64"],
                    expected["rewrite_name_fnv1a64"]):
            raise ValueError("RPG AREA1 behavior-three final state differs")
        if trace.get("transitions") != [{
                "module": "RPG.EXE", "input": "OC", "output": "--",
                "launched": True}]:
            raise ValueError("RPG AREA1 behavior-three replay missed OC entry")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 2:
            raise ValueError("RPG AREA1 behavior-three frame count differs")
        for page in pages:
            ...
            if sha256(compared_pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "RPG AREA1 behavior-three page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")
        print(
            "RPG AREA1 behavior three: east step from world (129,25) to (130,25), "
            "no Confirm/text event, two complete 320x200 RGB pages match "
            "the untouched original")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"RPG AREA1 behavior three: FAIL: {error}\n")

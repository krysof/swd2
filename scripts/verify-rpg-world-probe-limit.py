#!/usr/bin/env python3
"""Replay and lock BUIN1 four-cell probe limit."""
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed BUIN1 probe-limit digest {label}")


def u16(data: bytes, offset: int) -> int:
    if offset < 0 or offset + 2 > len(data):
        raise ValueError("MAPZ probe-limit oracle is out of bounds")
...
def mapz_oracle(mapz: bytes) -> tuple[list[int], list[int]]:
    ...
    if header != 512 or location != 4608 or area != 23549 or count != 11:
        raise ValueError("BUIN1 probe-limit MAPZ structure differs")
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_rpg_buin1_probe_limit_sequence" or \
                ...
                expected.get("player_world_position") != [10, 169] or \
                expected.get("player_direction") != 9 or \
                ...
                expected.get("forward_probe_cells") != \
                    [[11, 169], [12, 169], [13, 169], [14, 169]] or \
                expected.get("first_out_of_range_cell") != [15, 169] or \
                expected.get("event_dispatched") is not False or \
                expected.get("confirm_inputs") != 1 or \
                expected.get("text_boundaries") != 0 or \
                not isinstance(pages, list) or len(pages) != 2 or \
                [page.get("rewrite_frame") for page in pages] != [0, 1] or \
                [page.get("original_review_frame") for page in pages] != \
                    [238, 500]:
            raise ValueError("unsupported RPG BUIN1 probe-limit reference")
        ...
        if sha256(release_mapz) != expected["release_mapz_sha256"] or \
            ...
            raise ValueError("release BUIN1 probe-limit entity differs")
        ...
        if sha256(fixture_mapz) != expected["fixture_mapz_sha256"] or \
            ...
            raise ValueError("staged BUIN1 probe-limit entity differs")
        ...
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != save_digest or \
            ...
            raise ValueError("RPG BUIN1 probe-limit save triplets differ")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("RPG BUIN1 probe-limit input evidence differs")
        if expected.get("capture_wait_seconds") != 6.5 or \
                expected.get("capture_pace_seconds") != 1.2 or \
            ...
            raise ValueError("RPG BUIN1 probe-limit capture boundary differs")
        for name in (
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("RPG BUIN1 probe-limit replay timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    expected["rewrite_mapz_fnv1a64"],
                    expected["rewrite_name_fnv1a64"]):
            raise ValueError("RPG BUIN1 probe-limit final state differs")
        if trace.get("transitions") != [{
                "module": "RPG.EXE", "input": "OC", "output": "--",
                "launched": True}]:
            raise ValueError("RPG BUIN1 probe-limit replay missed OC entry")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 2:
            raise ValueError("RPG BUIN1 probe-limit frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "RPG BUIN1 probe-limit page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")
        if frames[0] != frames[1]:
            raise ValueError("RPG BUIN1 probe-limit Confirm changed the world page")
        print(
            "RPG BUIN1 probe limit: one Confirm leaves the entity at the "
            "fifth cell undispatched; before/after complete RGB pages match "
            "the untouched original")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"RPG BUIN1 probe limit: FAIL: {error}\n")

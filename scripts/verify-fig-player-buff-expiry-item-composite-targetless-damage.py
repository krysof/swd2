#!/usr/bin/env python3
"""Lock item 202's targetless composite damage anchor and expiry envelope."""
...
) + tuple(f"first_effect_page{index}" for index in range(1, 12)) + (
    "first_damage_flash",
) + tuple(f"first_damage_rise{index}" for index in range(1, 11)) + \
    tuple(f"second_effect_page{index}" for index in range(1, 11)) + (
        "second_damage_flash",
    ) + tuple(f"second_damage_rise{index}" for index in range(1, 11)) + (
        "restoration_step_four", "restoration_step_five",
        ...
        "round_boundary", "next_command",
    )
EXPECTED_REWRITE = (120,) + tuple(range(122, 169)) + tuple(range(173, 178)) + \
    (179,) + tuple(range(181, 188)) + tuple(range(189, 200)) + (201, 202)
REWRITE_ONLY_KINDS = (
    "item_command_boundary", "item_list_boundary", "first_darkening_step",
    "handler_dark_clean", "paid_dark_clean", "restoration_step_one",
    "restoration_step_two", "monster_pre_shake_clean",
    "final_shake_alternate", "monster_action_tail",
)
REWRITE_ONLY_FRAMES = (118, 119, 121, 169, 170, 171, 172, 180, 188, 200)
...
def digest(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed targetless-item digest {label}")
    return value
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_player_buff_expiry_after_item_composite_targetless_damage" or \
        ...
        for stem, offset in checks:
            ...
            if expected.get(stem + "_image_offset") != offset or \
                ...
                raise ValueError(
                    "FIG targetless-item instruction differs: " + stem)

        item_archive = (args.game / "ITEM.EXE").read_bytes()
        if sha256(item_archive) != expected["item_archive_sha256"]:
            raise ValueError("ITEM.EXE differs from targetless-item fixture")
        item = archive_record(item_archive, 202 + 2)
        monster = archive_record(item_archive, 500 + 2)
        if len(item) < 11 or \
                (int.from_bytes(item[0:2], "little"), item[6],
                 int.from_bytes(item[7:9], "little"), item[9], item[10]) != \
                    (0x10, 0, 0x6b, 0x39, 0x44) or \
                len(monster) < 0x2e or \
                int.from_bytes(monster[0x2c:0x2e], "little") != 1200:
            raise ValueError("ITEM.EXE targetless item/monster patch differs")
        if sha256((args.game / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("FIG targetless-item fixture differs")
        ...
        if sha256(autotype.read_bytes()) != expected["capture_autotype_sha256"] or \
            ...
            raise ValueError("FIG targetless-item inputs differ")
        if (expected.get("capture_wait_seconds"),
            ...
            raise ValueError("FIG targetless-item capture boundary differs")
        ...
        if not isinstance(limitation, str) or \
            ...
            raise ValueError("FIG targetless-item limitation is missing")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("FIG targetless-item rewrite trace differs")
        if trace.get("stop_reason") != "module requested exit" or \
                trace.get("final_marker") != "--" or trace.get("transitions") != [{
                    "module": "FIG.EXE", "input": "IF", "output": "--",
                    "launched": True}]:
            raise ValueError("FIG targetless-item replay exit differs")
        ...
        timed = (
            ...
            (137, "first_damage_flash_at_milliseconds"),
            (148, "second_nested_first_page_at_milliseconds"),
            (158, "second_damage_flash_at_milliseconds"),
            (175, "restored_at_milliseconds"),
            (176, "expiry_at_milliseconds"),
            (177, "clean_at_milliseconds"),
            (179, "monster_attack_at_milliseconds"),
            (200, "monster_tail_at_milliseconds"),
            (201, "round_boundary_at_milliseconds"),
            (202, "next_command_at_milliseconds"),
        )
        if any(times[index] != expected[key] for index, key in timed) or \
                times[177] - times[176] != expected["expiry_hold_milliseconds"] or \
                times[179] - times[177] != expected["clean_hold_milliseconds"]:
            raise ValueError("FIG targetless-item timing differs")
        voices = [entry for entry in trace["timeline"]
                  if entry.get("kind") == "voice"]
        voice_checks = (
            (7, "first_nested_voice_at_milliseconds", "15c15fbce02deca6"),
            (8, "second_nested_voice_at_milliseconds", "c76811d67d1cf7e9"),
            (9, "monster_voice_at_milliseconds", "7b151dcc4eff4103"),
        )
        if len(voices) != 10 or any(
            ...
            raise ValueError("FIG targetless-item voices differ")

        frames = load_indexed_frames(frame_path)
        if len(frames) != expected["rewrite_video"]["frames"]:
            raise ValueError("FIG targetless-item frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "FIG targetless-item exact page differs: " + page["kind"])
            ...
            if not isinstance(page.get("original_review_frame"), int) or \
                ...
                raise ValueError(
                    "FIG targetless-item original range differs: " + page["kind"])
        for page in rewrite_only:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "FIG targetless-item observation differs: " + page["kind"])

        print(
            "FIG targetless item composite expiry checkpoint: 74 exact pages "
            "lock the source-actor anchor, both damage bodies, shared outer "
            "envelope and same-turn expiry")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG targetless item composite expiry checkpoint: FAIL: {error}\n")

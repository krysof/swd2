...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != "original_fig_introduction_without_prompt" or \
                expected.get("formation_directory_offset") != 0x094 or \
        ...
        print(
            "FIG no-prompt introduction: bare enemy page, first glyph, all four "
            "animated cursor frames, command page, and seven exact original "
            "RGB pages")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG no-prompt introduction: FAIL: {error}\n")

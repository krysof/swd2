#!/usr/bin/env python3
"""Replay and lock FIG captured-ally 1048's card-free mediator flight."""
...
PAGE_FRAMES = tuple(range(52, 79))
PAGE_KINDS = (
    "ally_ability_action_card",
    *(f"af_medium_flight_step_{index:02d}" for index in range(1, 27)),
)
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed ally-medium digest {label}")
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_captured_ally_medium_install" or \
                expected.get("status") != "exact_rgb_checkpoint" or \
                expected.get("formation_directory_offset") != 392 or \
                expected.get("captured_item_id") != 374 or \
                expected.get("captured_ally_ability_id") != 12 or \
                ...
                tuple(page.get("rewrite_frame") for page in pages) != \
                    PAGE_FRAMES or \
                tuple(page.get("kind") for page in pages) != PAGE_KINDS:
            raise ValueError("unsupported FIG captured-ally medium reference")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from ally-medium reference")
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
                expected["fixture_save_sha256"]:
            raise ValueError("FIG ally-medium staged save differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("FIG ally-medium input evidence differs")
        if expected.get("capture_wait_seconds") != 5 or \
            ...
            raise ValueError("FIG ally-medium capture boundary differs")
        ...
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "FIG ally-medium page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")

        print(
            "FIG captured-ally medium checkpoint: action card and 26 AF "
            "flight positions remain card-free and match original RGB")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG captured-ally medium checkpoint: FAIL: {error}\n")

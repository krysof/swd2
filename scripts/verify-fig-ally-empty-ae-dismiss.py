#!/usr/bin/env python3
"""Replay and lock captured-ally special-B empty-AE dismissal."""
...
PAGE_KINDS = (
    "bare_before_empty_ae_dismissal",
    "ally_empty_ae_dismissal_action_card",
    "clean_after_empty_ae_dismissal",
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed ally-empty-AE-dismiss digest {label}")
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_captured_ally_empty_ae_dismissal" or \
                ...
                fixture_patch != (
                    ...
                    "special-B=53; FIG.EXE and ability 53 are unmodified") or \
                ...
                tuple(page.get("rewrite_frame") for page in pages) != \
                    PAGE_INDICES:
            raise ValueError(
                "unsupported FIG captured-ally empty-AE-dismiss reference")

        if sha256((args.source_game / "FIG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from ally empty-AE-dismiss reference")
        if sha256((args.fixture / "SAVE.DA1").read_bytes()) != \
                expected["fixture_save_sha256"]:
            raise ValueError("FIG captured-ally empty-AE staged save differs")
        if sha256((args.source_game / "ITEM.EXE").read_bytes()) != \
            ...
            raise ValueError("FIG captured-ally empty-AE ITEM fixture differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("FIG captured-ally empty-AE-dismiss inputs differ")
        if expected.get("capture_harness_sha256") != \
            ...
            raise ValueError("FIG captured-ally empty-AE-dismiss capture boundary differs")
        ...
        if not isinstance(limitation, str) or \
            ...
            raise ValueError(
                "FIG captured-ally empty-AE fixture limitation missing")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("FIG captured-ally empty-AE-dismiss timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    expected["rewrite_mapz_fnv1a64"],
                    expected["rewrite_name_fnv1a64"]):
            raise ValueError("FIG captured-ally empty-AE-dismiss save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG captured-ally empty-AE-dismiss missed IF entry")
        frame_times = [
        ...
        if frame_times != expected["rewrite_frame_times_milliseconds"]:
            raise ValueError("FIG captured-ally empty-AE-dismiss frame timing differs")
        ...
        if call >= len(voices) or (
            ...
            raise ValueError("FIG captured-ally empty-AE-dismiss SP061 call differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != expected["rewrite_video"]["frames"]:
            raise ValueError("FIG captured-ally empty-AE-dismiss frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    f"FIG ally empty-AE-dismiss {page['kind']} page differs")
            digest(page.get("original_png_sha256"),
                   f"{page['kind']}/original_png_sha256")
            if not isinstance(page.get("original_review_frame"), int):
                raise ValueError("ally empty-AE-dismiss review frame differs")

        # Selector 3dh sees the initial x=50h AE sentinel, so there is no 3c15
        ...
        if frames[26] != frames[31] or \
                frame_times[30:33] != [2307, 3076, 3350] or \
                expected["dismiss_voice_at_milliseconds"] != 2801:
            raise ValueError("FIG captured-ally empty-AE-dismiss sentinel timing differs")
        clean = frames[31][0]
        for index in PAGE_INDICES[:3]:
            ...
            if any(pixels[y * 320:y * 320 + 12] !=
                ...
                raise ValueError("FIG ally empty-AE-dismiss retained a party card")

        print(
            "FIG captured-ally empty AE-dismiss checkpoint: special B 53 "
            "takes the 50h AE sentinel, nine-tick card, SP061 and five-tick clean "
            "tail; 4 stable RGB pages match original")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(
            1, f"FIG captured-ally empty AE-dismiss checkpoint: FAIL: {error}\n")

#!/usr/bin/env python3
"""Lock learned ability 90's shared composite-damage envelope and expiry."""
...
EXPECTED_KINDS = (
    "learned_pose_zero", "learned_pose_four", "darkening_step_two",
    "darkening_step_three", "darkening_step_four", "darkest_pose_four",
) + tuple(f"first_nested_page{index}" for index in range(1, 21)) + \
    tuple(f"second_nested_page{index}" for index in range(1, 19)) + (
        "restoration_step_three", "restoration_step_four",
        "restored_pose_four", "attack_buff_expired", "expiry_tail_clean",
        "monster_attack_card",
    ) + tuple(f"monster_shake_page{index}" for index in range(1, 8)) + \
    tuple(f"monster_damage_rise{index}" for index in range(1, 11)) + (
        "round_boundary", "next_command",
    )
EXPECTED_REWRITE = (121, 122, 124, 125, 126, 127) + \
    tuple(range(128, 148)) + tuple(range(148, 166)) + \
    (170, 171, 172, 173, 174, 176) + tuple(range(178, 185)) + \
    tuple(range(187, 197)) + (198, 199)
REWRITE_ONLY_KINDS = (
    "ability_list_boundary", "target_selector", "target_confirm_transition",
    "first_darkening_step", "intermediate_dark_pose_one",
    "intermediate_dark_pose_two", "restoration_step_one",
    "restoration_step_two", "monster_preparation", "post_card_clean",
    "final_shake_alternate", "pre_damage_reaction", "monster_action_tail",
    "quit_boundary",
)
REWRITE_ONLY_FRAMES = (
    118, 119, 120, 123, 166, 167, 168, 169, 175, 177, 185, 186, 197, 200,
)
...
def digest(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed learned-composite-expiry digest {label}")
    return value
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_player_buff_expiry_after_ability_composite_damage" or \
        ...
        for stem, offset in checks:
            ...
            if expected.get(stem + "_image_offset") != offset or \
                ...
                raise ValueError("FIG learned-composite instruction differs: " + stem)
        if sha256((args.game / "ITEM.EXE").read_bytes()) != \
                expected["item_archive_sha256"]:
            raise ValueError("ITEM.EXE differs from learned-composite reference")
        if sha256((args.game / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("FIG learned-composite fixture differs")
        ...
        if sha256(autotype.read_bytes()) != expected["capture_autotype_sha256"] or \
            ...
            raise ValueError("FIG learned-composite inputs differ")
        if (expected.get("capture_wait_seconds"),
            ...
            raise ValueError("FIG learned-composite capture boundary differs")
        ...
        if not isinstance(limitation, str) or \
            ...
            raise ValueError("FIG learned-composite limitation is missing")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("FIG learned-composite rewrite trace differs")
        if trace.get("stop_reason") != "module requested exit" or \
                trace.get("final_marker") != "--" or trace.get("transitions") != [{
                    "module": "FIG.EXE", "input": "IF", "output": "--",
                    "launched": True}]:
            raise ValueError("FIG learned-composite replay exit differs")

        times = [entry["at_milliseconds"] for entry in trace["timeline"]
                 if entry.get("kind") == "frame"]
        timed = ((121, "pose_zero_at_milliseconds"),
                 (122, "pose_four_at_milliseconds"),
                 (148, "second_nested_first_page_at_milliseconds"),
                 (172, "restored_at_milliseconds"),
                 (173, "expiry_at_milliseconds"),
                 (174, "clean_at_milliseconds"),
                 (176, "monster_attack_at_milliseconds"),
        ...
        if any(times[index] != expected[key] for index, key in timed) or \
                times[173] - times[172] != 55 or \
                times[174] - times[173] != expected["expiry_hold_milliseconds"] or \
                times[176] - times[174] != expected["clean_hold_milliseconds"]:
            raise ValueError("FIG learned-composite timing differs")
                ...
                (voices[9].get("at_milliseconds"),
                 voices[9].get("payload_fnv1a64")) != \
                    (expected["monster_voice_at_milliseconds"],
                     "7b151dcc4eff4103"):
            raise ValueError("FIG learned-composite voices differ")

        frames = load_indexed_frames(frame_path)
        if len(frames) != expected["rewrite_video"]["frames"]:
            raise ValueError("FIG learned-composite frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "FIG learned-composite exact page differs: " + page["kind"])
        ...
        for page in rewrite_only:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "FIG learned-composite observation differs: " + page["kind"])

        print(
            "FIG learned-composite expiry checkpoint: 69 exact pages lock "
            "pose0/pose4, the shared dark/payment envelope, expiry and the "
            "complete following monster response")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG learned-composite expiry checkpoint: FAIL: {error}\n")

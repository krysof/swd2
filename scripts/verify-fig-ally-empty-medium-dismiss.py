#!/usr/bin/env python3
"""Replay and lock captured-ally special-B empty-AF dismissal."""
...
PAGE_KINDS = (
    "bare_before_empty_dismissal",
    "ally_empty_dismissal_action_card",
    "clean_after_empty_dismissal",
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed ally-empty-dismiss digest {label}")
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_captured_ally_empty_medium_dismissal" or \
                ...
                tuple(page.get("rewrite_frame") for page in pages) != \
                    PAGE_INDICES:
            raise ValueError(
                "unsupported FIG captured-ally empty-dismiss reference")

        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from ally empty-dismiss reference")
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
                expected["fixture_save_sha256"]:
            raise ValueError("FIG captured-ally empty-dismiss staged save differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("FIG captured-ally empty-dismiss inputs differ")
        if expected.get("capture_harness_sha256") != \
            ...
            raise ValueError("FIG captured-ally empty-dismiss capture boundary differs")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("FIG captured-ally empty-dismiss timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    expected["rewrite_mapz_fnv1a64"],
                    expected["rewrite_name_fnv1a64"]):
            raise ValueError("FIG captured-ally empty-dismiss save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG captured-ally empty-dismiss missed IF entry")
        frame_times = [
        ...
        if frame_times != expected["rewrite_frame_times_milliseconds"]:
            raise ValueError("FIG captured-ally empty-dismiss frame timing differs")
        ...
        if call >= len(voices) or (
            ...
            raise ValueError("FIG captured-ally empty-dismiss SP061 call differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != expected["rewrite_video"]["frames"]:
            raise ValueError("FIG captured-ally empty-dismiss frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    f"FIG ally empty-dismiss {page['kind']} page differs")
            digest(page.get("original_png_sha256"),
                   f"{page['kind']}/original_png_sha256")
            if not isinstance(page.get("original_review_frame"), int):
                raise ValueError("ally empty-dismiss review frame differs")

        # Selector 3eh sees the initial x=50h sentinel, so there is no 3c15
        ...
        if frames[26] != frames[31] or \
            ...
            raise ValueError("FIG captured-ally empty-dismiss sentinel timing differs")
        clean = frames[31][0]
        for index in PAGE_INDICES[:3]:
            ...
            if any(pixels[y * 320:y * 320 + 12] !=
                ...
                raise ValueError("FIG ally empty-dismiss retained a party card")

        print(
            "FIG captured-ally empty medium-dismiss checkpoint: special B 67 "
            "takes the 50h sentinel, nine-tick card, SP061 and five-tick clean "
            "tail; 4 stable RGB pages match original")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(
            1, f"FIG captured-ally empty medium-dismiss checkpoint: FAIL: {error}\n")

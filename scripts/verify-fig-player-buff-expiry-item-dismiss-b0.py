#!/usr/bin/env python3
"""Lock FIG same-turn attack-buff expiry after item 223 dismisses B0."""
...
EXPECTED_KINDS = (
    "item_pose_zero_with_b0_medium", "darkening_step_two", "darkening_step_three",
    ...
    "restoration_step_four", "restored_pose_zero_with_b0_medium",
...
EXPECTED_REWRITE = (
    183, 185, 186, 187, 188, 190, 192, 196, 198, 199, 200, 201,
    202, 203, 204,
) + tuple(range(205, 212)) + tuple(range(214, 224)) + (224, 225, 226)
EXPECTED_ORIGINAL = (
    6292, 6302, 6306, 6310, 6314, 6347, 6351, 6355, 6359, 6361,
    6363, 6365, 6398, 6471, 6494, 6512, 6514, 6516, 6518, 6520,
    6522, 6524, 6528, 6530, 6532, 6534, 6536, 6538, 6540, 6542,
    6544, 6548, 6549, 6581, 6603,
)
EXPECTED_STABLE = (
    6300, 6304, 6308, 6312, 6344, 6348, 6352, 6355, 6359, 6361,
    6363, 6397, 6470, 6493, 6511, 6513, 6515, 6517, 6519, 6521,
    6523, 6527, 6528, 6530, 6532, 6534, 6536, 6538, 6540, 6542,
    6546, 6548, 6580, 6601, 6786,
)
REWRITE_ONLY_KINDS = (
...
REWRITE_ONLY_FRAMES = (184, 189, 191, 193, 195, 197, 212, 213)
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_player_buff_expiry_after_item_b0_dismissal" or \
                ...
                (expected.get("medium_slot"),
                 ...
                 expected.get("expiry_player_turn")) != (2, 0xb0, 5) or \
        ...
        if not isinstance(limitation, str) or \
                ...
                not all(token in limitation for token in (
                    "0a84", "11fc", "1235", "47fa", "4894", "482e",
                    "0c41", "0da7", "0d98")) or \
                "Repeated final right/confirm" not in limitation:
        ...
        timed = (
            (183, "item_pose_at_milliseconds"),
            (188, "darkest_pose_at_milliseconds"),
            (189, "first_dismiss_flip_at_milliseconds"),
            (196, "last_dismiss_flip_at_milliseconds"),
            (197, "first_restoration_at_milliseconds"),
            (201, "restored_pose_at_milliseconds"),
            (202, "expiry_at_milliseconds"),
            (203, "clean_at_milliseconds"),
            (204, "monster_attack_at_milliseconds"),
            (224, "monster_tail_at_milliseconds"),
            (225, "round_boundary_at_milliseconds"),
            (226, "next_command_at_milliseconds"),
        )
        if any(times[index] != expected[key] for index, key in timed) or \
                expected.get("dismiss_voice_at_milliseconds") != 14518 or \
                any(times[index + 1] - times[index] != 55
                    for index in range(189, 201)) or \
                expected.get("dismiss_flip_milliseconds") != 55 or \
                times[202] - times[201] != 55 or \
                times[203] - times[202] != 989 or \
                expected.get("expiry_hold_milliseconds") != 989 or \
                times[204] - times[203] != 275 or \
                expected.get("clean_hold_milliseconds") != 275 or \
                times[226] - times[225] != 110:

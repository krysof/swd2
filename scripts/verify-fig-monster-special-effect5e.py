#!/usr/bin/env python3
"""Lock shipped monster special ability 65 / effect 5E and its target anchor."""
...
EXCLUDED = [0, 4, 13, 17, 18, 36, 40, 57]
...
def digest(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed special-effect-5E digest {label}")
    return value
...
def main() -> int:
    ...
    try:
        ...
        if reference.get("schema_version") != 1 or \
                reference.get("kind") != \
                    "original_fig_monster_special_effect5e_sequence" or \
                ...
                tuple(reference.get(name) for name in (
                    ...
                    (0x10, 512, 65, 0x5E, 0xA403) or \
                tuple(reference.get(name) for name in (
                    "formation_mode_x_column", "monster_sprite_width_pixels",
                    "converted_mode_x_width",
                    "original_target_center_x_pixels")) != \
                    (23, 116, 29, 148) or \
                reference.get("excluded_rewrite_frames") != EXCLUDED or \
                "No equality is claimed" not in \
                    reference.get("excluded_reason", "") or \
                not isinstance(pages, list) or len(pages) != 50 or \
                [page.get("rewrite_frame") for page in pages] != wanted:
            raise ValueError("unsupported FIG special-effect-5E reference")
        ...
        if any(not isinstance(value, int) or value <= 0
            ...
            raise ValueError("special-effect-5E original page order differs")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                reference["reference_program_sha256"] or \
                sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
                reference["fixture_save_sha256"]:
            raise ValueError("special-effect-5E program or fixture differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("special-effect-5E input evidence differs")
        if (reference.get("capture_harness_sha256"),
                ...
                reference.get("capture_review_frames")) != (
                    ...
                    5, 1, 30, 2099):
            raise ValueError("special-effect-5E capture boundary differs")
        ...
        if trace.get("input") != reference["rewrite_input"] or \
            ...
            raise ValueError("special-effect-5E deterministic trace differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("special-effect-5E replay missed IF entry")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 58:
            raise ValueError("special-effect-5E frame boundary differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "special-effect-5E page differs from original: " +
                    str(page["rewrite_frame"]))
            digest(page.get("original_png_sha256"),
                   f"frame {page['rewrite_frame']}/original_png_sha256")
        print(
            "FIG special effect-5E checkpoint: ability 65 status and its "
            "Mode-X target anchor match 50 chronological original RGB pages")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG special-effect-5E checkpoint: FAIL: {error}\n")

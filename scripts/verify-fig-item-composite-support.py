#!/usr/bin/env python3
"""Replay and lock type-10 item 219's targetless two-buff presentation."""
...
EXPECTED_PAGES = (
    "item_pose0", "first_buff_card", "second_buff_card", "post_buffs_pose",
    "monster_action_card", "player_action_boundary",
    "next_command",
)
...
def digest(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed composite-support digest {label}")
    return value
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                "original_fig_type10_composite_support_item" or \
        ...
        if sha256(autotype.read_bytes()) != expected["capture_autotype_sha256"] or \
            ...
            raise ValueError("FIG composite-support inputs differ")
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
                expected["fixture_save_sha256"]:
            raise ValueError("FIG composite-support staged save differs")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("FIG composite-support timeline differs")
        if trace.get("frame_fnv1a64", [])[-1:] != [
            ...
            raise ValueError("FIG composite-support final state differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG composite-support did not use IF entry")
        ...
        if len(frames) != expected["rewrite_video"]["frames"] or \
            ...
            raise ValueError("FIG composite-support page set differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    f"FIG composite-support {page['kind']} differs from original")
            digest(page.get("original_png_sha256"), "original_png_sha256")
        for name in ("capture_harness_sha256", "capture_video_sha256",
                     "capture_manifest_sha256"):
            digest(expected.get(name), name)
        print(
            "FIG composite-support checkpoint: item 219 targets its caller, "
            "keeps both 66h/69h cards, and reaches the next command page")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG composite-support checkpoint: FAIL: {error}\n")

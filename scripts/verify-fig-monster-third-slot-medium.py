#!/usr/bin/env python3
"""Replay and lock FIG's third live monster installing the B0 mediator."""
...
EXPECTED_FRAMES = tuple(range(1, 21))
EXPECTED_KINDS = (
    "initial_command_page", "attack_command_selected",
    "three_monster_target_selector", "slot2_medium_prepare_bare",
    "fixed_b0_medium_name_card",
    *(f"slot2_medium_flight_step_{index:02d}" for index in range(1, 14)),
    "monster_cleanup_with_installed_b0",
    "player_pose0_successor_with_b0",
)
...
def digest(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed monster-third-slot-medium digest {label}")
    return value
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_third_monster_b0_medium_install" or \
                ...
                expected.get("caster_monster_index") != 2 or \
                expected.get("caster_definition_id") != 385 or \
                ...
                expected.get("monster_primary_ability") != 118 or \
                expected.get("requested_medium_index") != 2 or \
                expected.get("fixed_summon_name_ability_id") != 82 or \
            ...
            raise ValueError("unsupported FIG monster-third-slot-medium reference")

        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from monster-third-slot-medium reference")
        if sha256((args.game / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("monster-third-slot-medium staged game differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("monster-third-slot-medium input evidence differs")
        if expected.get("capture_wait_seconds") != 5 or \
                expected.get("capture_pace_seconds") != 0.7 or \
            ...
            raise ValueError("monster-third-slot-medium capture boundary differs")
        for name in ("capture_video_sha256", "capture_manifest_sha256"):
            digest(expected.get(name), name)
        limitation = expected.get("capture_limitation")
        if not isinstance(limitation, str) or \
                "20 registered pages" not in limitation or \
                "stable full-frame RGB" not in limitation:
            raise ValueError("monster-third-slot-medium capture limitation missing")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("monster-third-slot-medium replay boundaries differ")
        if trace.get("video") != expected["rewrite_video"] or \
            ...
            raise ValueError("monster-third-slot-medium audio/video timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    expected["rewrite_mapz_fnv1a64"],
                    expected["rewrite_name_fnv1a64"]):
            raise ValueError("monster-third-slot-medium final save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("monster-third-slot-medium replay missed IF entry")
        ...
        if frame_times != tuple(expected["rewrite_frame_times_milliseconds"]):
            raise ValueError("monster-third-slot-medium frame timing differs")
        voices = [
            {"call": item.get("call"),
        ...
        if voices != expected["rewrite_voice_timeline"]:
            raise ValueError("monster-third-slot-medium voice timeline differs")
        ...
        if not isinstance(medium_call, int) or medium_call >= len(voices) or \
            ...
            raise ValueError("monster-third-slot-medium SP049 boundary differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != expected["rewrite_video"]["frames"]:
            raise ValueError("monster-third-slot-medium frame count differs")
        previous_original = -1
        for page in pages:
            review = page.get("original_review_frame")
            if not isinstance(review, int) or review <= previous_original:
                raise ValueError("monster-third-slot-medium original order differs")
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "monster-third-slot-medium page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")

        print(
            "FIG monster-third-slot-medium checkpoint: live slot-2 prepare, fixed B0 "
            "name card, 13 mediator-flight positions, installed-medium cleanup "
            "and player successor match 20 original RGB pages")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG monster-third-slot-medium checkpoint: FAIL: {error}\n")

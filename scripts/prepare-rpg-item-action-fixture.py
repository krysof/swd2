#!/usr/bin/env python3
"""Create the deterministic save used by the four-way Item action replay."""
...
def main() -> int:
    ...
    for slot in range(50):
        set_word(save, 0x382 + slot * 2, 68 if slot == 0 else 0)
    for counter in range(5):
        set_word(save, 0x3e6 + counter * 2, 0)
    save[0x3f1] = 0  # expose the fourth / lower alchemy-pot card
    set_word(save, 0x104, 1000)

    # Use the same stable SWRO7 page as the shop references, but enter Item
    # through the field diamond rather than interacting with the merchant.
    ...
    print("RPG Item action fixture: item 68 and all four direction cards")
    return 0

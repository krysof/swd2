...
POST_REWRITE = (153, 154) + tuple(range(155, 162)) + (163,) + \
    tuple(range(164, 174)) + (174, 175, 176)
POST_ORIGINAL = (3746, 3769, 3787, 3789, 3791, 3793, 3795, 3797, 3799,
                 3802, 3803, 3805, 3807, 3809, 3811, 3813, 3815, 3817,
                 3819, 3823, 3825, 3857, 3878)
POST_STABLE = (3768, 3786, 3788, 3790, 3792, 3794, 3796, 3798, 3801,
...
REWRITE_ONLY_KINDS = (
    ...
    "attack_buff_expired", "final_monster_shake_clean",
)
REWRITE_ONLY_FRAMES = (121, 126, 146, 147, 148, 149, 150, 151, 152, 162)
...
def main() -> int:
    ...
    try:
        ...
        if changed_box(frames[151][0], frames[152][0]) != (
                ...
                expected["changed_from_restored_medium_pixels"] != 5463 or \
                expected["changed_from_restored_medium_bbox"] != \
                    [0, 1, 317, 198] or \
                expected["changed_from_expiry_to_clean_pixels"] != 4864 or \
        ...
        print(
            "FIG item-medium expiry checkpoint: 47 original RGB pages lock "

#!/usr/bin/env python3
"""Lock FIG same-turn attack-buff expiry after learned AF dismissal."""
...
EXPECTED_KINDS = (
    "pose_zero_with_af_medium", "pose_four_with_af_medium",
    ...
    "restoration_step_four", "restored_pose_four_with_af_medium",
...
EXPECTED_REWRITE = (
    163, 164, 166, 167, 168, 169, 171, 173, 177, 179, 180, 181,
    182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 192,
) + tuple(range(195, 205)) + (205, 206, 207)
EXPECTED_ORIGINAL = (
    5325, 5337, 5347, 5351, 5355, 5358, 5391, 5395, 5399, 5403,
...
REWRITE_ONLY_FRAMES = (165, 170, 172, 174, 176, 178, 193, 194)
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_player_buff_expiry_after_learned_af_dismissal" or \
        ...
        if not isinstance(limitation, str) or \
                ...
                not all(token in limitation for token in (
                    "0a97", "4338", "4377", "47c6", "4886", "482e",
                    "0c41", "0da7", "0d98")) or \
                "duplicated final LEFT" not in limitation:
        ...
        timed = (
            (163, "pose_zero_at_milliseconds"),
            (164, "pose_four_at_milliseconds"),
            (169, "darkest_pose_at_milliseconds"),
            (170, "first_dismissal_flip_at_milliseconds"),
            (177, "last_dismissal_flip_at_milliseconds"),
            (182, "restored_pose_at_milliseconds"),
            (183, "expiry_at_milliseconds"),
            (184, "clean_at_milliseconds"),
            (185, "monster_attack_at_milliseconds"),
            (205, "monster_tail_at_milliseconds"),
            (206, "round_boundary_at_milliseconds"),
            (207, "next_command_at_milliseconds"),
        )
        if any(times[index] != expected[key] for index, key in timed) or \
                any(times[index + 1] - times[index] != 55
                    for index in range(170, 177)) or \
        ...
        # The last restored 4417 page still contains medium AF. 0c41 must
        ...
        if changed_box(frames[182][0], frames[183][0]) != (
                ...
                expected["changed_from_expiry_to_clean_bbox"] != \
                    [12, 120, 91, 199] or \
                frames[182] == frames[183] or frames[183] == frames[184] or \
                frames[184] != frames[206]:
            raise ValueError(
                "FIG learned-medium expiry lost pose or inserted early cleanup")

        print(
            "FIG learned-medium expiry checkpoint: 36 original RGB pages "
            "lock AF dismissal, retained pose four, same-turn expiry and the "

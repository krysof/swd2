#!/usr/bin/env python3
"""Stage BUIN1's released side-ray interaction entity in isolation."""
...
EXPECTED_SAVE_SHA256 = \
    "3033e8f96dd0f756e6493bf63ba1451f3973ccc151caef6736bb7c31d2c8e4bb"
...
def main() -> int:
    ...
    try:
        ...
        if actual_mapz != EXPECTED_MAPZ_SHA256:
            raise ValueError(f"side-ray MAPZ differs: {actual_mapz}")
        ...
        # (18,168), face west and issue Confirm: the centre ray stays empty,
        # then DATA:3cbf offsets the second ray south to find the entity.
        for offset, value in (
                (0x408, 96), (0x40d, 55808), (0x40f, 8), (0x424, 268),
                (0x012, 34), (0x02a, 88), (0x0a2, 6),
                (0x41b, 0), (0x41d, 155)):
        ...
        if actual_save != EXPECTED_SAVE_SHA256:
            raise ValueError(f"side-ray SAVE differs: {actual_save}")
        ...
        print(f"RPG BUIN1 side-ray fixture: SAVE {actual_save}, MAPZ {actual_mapz}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(1, f"RPG BUIN1 side-ray fixture: FAIL: {error}\n")

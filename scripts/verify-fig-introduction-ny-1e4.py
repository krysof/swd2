#!/usr/bin/env python3
"""Replay and prove FIG's generic NY introduction and prompt boundary."""
...
def main() -> int:
    ...
    try:
        ...
        rewrite_frames = [0, 1, 3, 4, 5, 6, 9]
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != "original_fig_introduction_ny_prompt_1e4" or \
                expected.get("formation_directory_offset") != 0x1E4 or \
        ...
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("FIG 1e4h introduction staged save differs")
        ...
        subprocess.run([
            ...
            str(args.reference.with_name("replay-fig-introduction-ny-1e4.txt")),
        ...
        if trace.get("input") != {
            ...
            raise ValueError("FIG 1e4h introduction input boundaries differ")
        if trace.get("video") != {
                "frames": 10, "direct_updates": 2,
                "last_width": 320, "last_height": 200,
                "fnv1a64": expected["rewrite_video_fnv1a64"]}:
            raise ValueError("FIG 1e4h introduction video differs")
        if trace.get("audio") != {
            ...
            raise ValueError("FIG 1e4h introduction audio or timing differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("FIG 1e4h introduction final state differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "OC",
                "launched": True}]:
            raise ValueError("FIG 1e4h introduction return boundary differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 10:
            raise ValueError("FIG 1e4h introduction frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    f"FIG 1e4h introduction {page['kind']} no longer matches original")
        ...
        print(
            "FIG 1e4h NY introduction: bare enemy page, first glyph, all four "
        ...
        parser.exit(1, f"FIG 1e4h NY introduction: FAIL: {error}\n")

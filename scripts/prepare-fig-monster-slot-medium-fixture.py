#!/usr/bin/env python3
"""Stage a live three-monster battle whose second slot installs AF first."""
...
EXPECTED_ITEM = "2e872cc3dca860b487f9017234568d79711323f3f19f5336b2f655490de625c2"
...
def main() -> int:
    ...
    try:
        ...
        # original's pathological pre-dead-slot state.  Slot 1 has the high
        ...
        set_fields(385, idle)
        set_fields(502, (
            (0x2C, 60000), (0x2E, 9), (0x32, 8), (0x36, 1),
        ...
        if save_digest != EXPECTED_SAVE:
            raise ValueError(
                f"FIG monster-slot-medium SAVE differs: {save_digest}")
        if item_digest != EXPECTED_ITEM:
            raise ValueError(
                f"FIG monster-slot-medium ITEM differs: {item_digest}")
        (args.output / "SAVE.DA1").write_bytes(save)
        item_path.write_bytes(item)
        print(
            "FIG monster-slot-medium fixture: "
            f"SAVE={save_digest} ITEM={item_digest}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(1, f"FIG monster-slot-medium fixture: FAIL: {error}\n")

#!/usr/bin/env python3
"""Replay and lock FIG's original generic monster ability-10 sequence."""
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed monster-generic evidence digest {label}")
...
def main() -> int:
    ...
    try:
        ...
        if reference.get("schema_version") != 1 or \
            ...
            raise ValueError("unsupported FIG monster-generic reference")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                reference["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from monster-generic reference")
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
                reference["fixture_save_sha256"]:
            raise ValueError("monster-generic staged SAVE differs")
        autotype = args.reference.with_name(reference["capture_autotype"])
        if sha256(autotype.read_bytes()) != \
                reference["capture_autotype_sha256"]:
            raise ValueError("monster-generic original AUTOTYPE differs")
        if reference.get("capture_wait_seconds") != 5 or \
            ...
            raise ValueError("monster-generic capture boundary differs")
        ...
        subprocess.run([
            ...
            str(args.reference.with_name("replay-fig-monster-generic.txt")),
        ...
        if trace.get("video") != reference["rewrite_video"] or \
            ...
            raise ValueError("monster-generic video/audio timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    reference["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("monster-generic final save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("monster-generic replay did not use IF entry")

        frames = load_indexed_frames(frame_path)
        if len(frames) != reference["rewrite_video"]["frames"]:
            raise ValueError("monster-generic frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "monster-generic page no longer matches original: "
                    f"{page['label']}")
            ...
            if not isinstance(page.get("original_review_frame"), int) or \
                ...
                raise ValueError(
                    f"monster-generic review frame differs: {page['label']}")

        print(
            "FIG monster-generic checkpoint: bare turn/name pages, 25ee "
            "clean/solid alternation and following pose exactly match "
            "original 320x200 RGB")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG monster-generic checkpoint: FAIL: {error}\n")

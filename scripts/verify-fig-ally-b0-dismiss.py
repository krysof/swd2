...
PAGE_KINDS = (
    ...
    "ally_clean_after_b0_removal",
)
PAGE_INDICES = tuple(range(67, 79))
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_captured_ally_b0_dismissal" or \
                ...
                tuple(page.get("rewrite_frame") for page in pages) != \
                    PAGE_INDICES:
            raise ValueError("unsupported FIG captured-ally B0 dismissal reference")

        if sha256((args.source_game / "FIG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from captured-ally B0 dismissal reference")
        if sha256((args.fixture / "SAVE.DA1").read_bytes()) != \
                expected["fixture_save_sha256"]:
            raise ValueError("FIG captured-ally B0 dismissal staged save differs")
        if sha256((args.source_game / "ITEM.EXE").read_bytes()) != \
            ...
            raise ValueError("FIG captured-ally B0 dismissal ITEM fixture differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("FIG captured-ally B0 dismissal input evidence differs")
        if expected.get("capture_harness_sha256") != \
            ...
            raise ValueError("FIG captured-ally B0 dismissal capture boundary differs")
        ...
        if not isinstance(limitation, str) or \
            ...
            raise ValueError("FIG captured-ally B0 dismissal limitation missing")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("FIG captured-ally B0 dismissal timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    expected["rewrite_mapz_fnv1a64"],
                    expected["rewrite_name_fnv1a64"]):
            raise ValueError("FIG captured-ally B0 dismissal save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG captured-ally B0 dismissal missed IF entry")
        frame_times = [
        ...
        if frame_times != expected["rewrite_frame_times_milliseconds"]:
            raise ValueError("FIG captured-ally B0 dismissal frame timing differs")
        ...
        if install_call >= len(voices) or (
            ...
            raise ValueError("FIG captured-ally B0 dismissal SP049 call differs")
        ...
        if call >= len(voices) or (
            ...
            raise ValueError("FIG captured-ally B0 dismissal SP061 call differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != expected["rewrite_video"]["frames"]:
            raise ValueError("FIG captured-ally B0 dismissal frame count differs")
        exact_kinds = []
        for page in pages:
        ...
        if tuple(exact_kinds) != tuple(
            ...
            raise ValueError("FIG captured-ally B0 dismissal evidence set differs")

        # The four bare halves are the same installed-B0 2db8 page; each
        ...
        if any(frames[index][0] != frames[69][0]
               for index in (71, 73, 75, 77)) or \
                any(frames[index][0] != frames[70][0]
                    for index in (72, 74, 76)):
            raise ValueError("FIG captured-ally B0 dismissal page alternation differs")
        stencil = frames[70][0]
        card = frames[68][0]
        changed = [
            (x, y) for y in range(1, 49) for x in range(240, 280)
            if stencil[y * 320 + x] != card[y * 320 + x]
        ]
        if len(changed) != 920 or \
                min(x for x, _ in changed) != 249 or \
                max(x for x, _ in changed) != 269 or \
                any(stencil[y * 320 + x] != 0x0f for x, y in changed):
            raise ValueError("FIG captured-ally B0 mode-0 stencil differs")
        clean = frames[78][0]
        for index in PAGE_INDICES:
            ...
            if any(pixels[y * 320:y * 320 + 12] !=
                ...
                raise ValueError("FIG captured-ally B0 dismissal retained a party card")

        print(
            "FIG captured-ally B0-dismiss checkpoint: special B 83 removes "
            "B0 through the card-free 1048 action, SP061 and eight 65f1 flips; "
            "11 stable RGB pages match original")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(
            1, f"FIG captured-ally B0-dismiss checkpoint: FAIL: {error}\n")

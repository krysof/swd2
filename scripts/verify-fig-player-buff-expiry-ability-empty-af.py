#!/usr/bin/env python3
"""Lock FIG same-turn attack-buff expiry after empty AF dismissal."""
...
EXPECTED_ORIGINAL = (
    4263, 4275, 4285, 4289, 4293, 4297, 4375, 4446, 4467, 4485,
    4487, 4489, 4491, 4493, 4495, 4497, 4501, 4503, 4505, 4507,
    4509, 4511, 4513, 4515, 4517, 4521, 4523, 4555, 4576,
)
EXPECTED_STABLE = (
    4274, 4283, 4287, 4291, 4295, 4334, 4445, 4463, 4484, 4486,
    4488, 4490, 4492, 4494, 4496, 4500, 4502, 4504, 4506, 4508,
    4510, 4512, 4514, 4516, 4520, 4522, 4554, 4575, 5599,
)
REWRITE_ONLY_KINDS = (
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_player_buff_expiry_after_empty_af_medium_dismissal" or \
                ...
                (expected.get("dismiss_ability_id"),
                 expected.get("dismiss_effect_code")) != (67, 0x3e) or \
        ...
        if not isinstance(limitation, str) or \
                ...
                not all(token in limitation for token in (
                    "0a97", "4338", "4377", "4886", "482e", "0c41", "0da7",
                    "0d98")):
        ...
        if any(times[index] != expected[key] for index, key in timed) or \
                expected.get("dismiss_voice_at_milliseconds") != 10558 or \
                times[129] - 10558 != 275 or \
                expected.get("empty_slot_hold_milliseconds") != 275 or \
                times[134] - times[133] != 55 or \
                times[135] - times[134] != 989 or \
                expected.get("expiry_hold_milliseconds") != 989 or \
                times[136] - times[135] != 275 or \
                expected.get("clean_hold_milliseconds") != 275 or \
                times[158] - times[157] != 110:

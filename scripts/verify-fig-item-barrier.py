#!/usr/bin/env python3
"""Replay and lock direct item 204's active barrier dispatcher path."""
...
EXPECTED_PAGES = (
    "item_pose0", "darkened_item_pose0", "ordinary_target_card",
    "sp338_frame0", "sp338_frame1", "sp338_frame2", "sp338_frame3",
)
RESTORATION_FRAMES = tuple(range(37, 44))
...
def digest(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed item-barrier digest {label}")
    return value
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != "original_fig_direct_item_barrier" or \
                ...
                (expected.get("item_id"), expected.get("dispatcher_index"),
                 ...
                 expected.get("effect_code")) != (204, 64, 64, 0x62) or \
                expected.get("capture_wait_seconds") != 5 or \
                expected.get("capture_pace_seconds") != 1 or \
                expected.get("capture_time_limit_seconds") != 15:
            raise ValueError("unsupported FIG item-barrier reference")
        limitation = expected.get("capture_limitation")
        if not isinstance(limitation, str) or \
                "585e/4417/0d98" not in limitation or \
                "straddle live VGA DAC writes" not in limitation:
            raise ValueError("FIG item-barrier evidence boundary differs")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from the item-barrier reference")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("FIG item-barrier inputs differ")
        if sha256((args.save_dir / "SAVE.DA1").read_bytes()) != \
                expected["fixture_save_sha256"]:
            raise ValueError("FIG item-barrier staged save differs")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("FIG item-barrier timeline differs")
        if trace.get("frame_fnv1a64", [])[-1:] != [
            ...
            raise ValueError("FIG item-barrier state differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG item-barrier did not use IF entry")
        ...
        if len(frames) != expected["rewrite_video"]["frames"] or \
            ...
            raise ValueError("FIG item-barrier page set differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    f"FIG item-barrier {page['kind']} differs from original")
            digest(page.get("original_png_sha256"),
                   f"{page.get('kind')}/original_png_sha256")
            if not isinstance(page.get("original_review_frame"), int):
                raise ValueError("FIG item-barrier review frame differs")
        ...
        if not isinstance(restoration, list) or tuple(
                ...
                RESTORATION_FRAMES:
            raise ValueError("FIG item-barrier restoration set differs")
        for page in restoration:
            ...
            if sha256(pixels) != page["indexed_sha256"] or \
                ...
                raise ValueError("FIG item-barrier restoration differs")

        for name in ("capture_harness_sha256", "capture_video_sha256",
                     "capture_manifest_sha256"):
            digest(expected.get(name), name)
        print(
            "FIG direct item-barrier checkpoint: item 204 pose0, 43ce fade, "
            "ordinary target card, SP338, deferred debit, and tail match")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG item-barrier checkpoint: FAIL: {error}\n")

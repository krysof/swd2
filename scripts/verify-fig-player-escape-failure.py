#!/usr/bin/env python3
"""Replay and lock FIG's failed player-escape action boundary."""
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed FIG player-escape-failure digest {label}")
...
def main() -> int:
    ...
    try:
        ...
        if reference.get("schema_version") != 1 or \
                reference.get("kind") != \
                    "original_fig_player_escape_failure_checkpoint" or \
                ...
                tuple(page.get("rewrite_frame") for page in pages) != \
                    PAGE_FRAMES:
            raise ValueError("unsupported FIG player-escape-failure reference")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                reference["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from escape-failure reference")
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != \
                reference["fixture_save_sha256"]:
            raise ValueError("FIG escape-failure staged SAVE differs")
        if sha256((args.game / "ORC.EXE").read_bytes()) != \
                reference["fixture_orc_sha256"]:
            raise ValueError("FIG escape-failure staged ORC differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("FIG escape-failure original AUTOTYPE differs")
        if sha256(replay.read_bytes()) != reference["replay_input_sha256"]:
            raise ValueError("FIG escape-failure replay input differs")
        if reference.get("capture_wait_seconds") != 5 or \
            ...
            raise ValueError("FIG escape-failure capture boundary differs")
        ...
        if trace.get("video") != reference["rewrite_video"] or \
            ...
            raise ValueError("FIG escape-failure timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    reference["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("FIG escape-failure final save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG escape-failure replay missed IF entry")
        ...
        if len(voices) != 1 or (
                ...
                voices[0].get("payload_fnv1a64")) != (
                    0, reference["attack_voice_at_milliseconds"],
                    reference["attack_voice_payload_fnv1a64"]):
            raise ValueError("FIG escape-failure SP106 checkpoint differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != reference["rewrite_video"]["frames"]:
            raise ValueError("FIG escape-failure frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "FIG escape-failure page differs from original: " +
                    page["label"])
            digest(page.get("original_png_sha256"),
                   page["label"] + "/original_png_sha256")

        print(
            "FIG escape-failure checkpoint: escape poses/card, 0d98 bare "
            "five-tick boundary and following monster turn match original RGB")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG escape-failure checkpoint: FAIL: {error}\n")

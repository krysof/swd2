#!/usr/bin/env python3
"""Stage a fixed FIG battle whose nonfatal player attack is followed by monster generic ability 54's missing AE medium install."""
...
EXPECTED_ITEM = "aa518e6e874532c64e9b4e2c2e2a558649fb39b0678ee61fdc32fd999ae1359e"
...
def main() -> int:
    ...
    try:
        ...
        word(item, header_size + record + 0x32, 54)
        ...
        if save_digest != EXPECTED_SAVE:
            raise ValueError(f"FIG player-attack-medium SAVE differs: {save_digest}")
        if item_digest != EXPECTED_ITEM:
            raise ValueError(f"FIG player-attack-medium ITEM differs: {item_digest}")
        (args.output / "SAVE.DA1").write_bytes(save)
        item_path.write_bytes(item)
        print(
            "FIG player-attack-medium fixture: "
            f"SAVE={save_digest} ITEM={item_digest}")
        return 0
    except (OSError, ValueError) as error:
        parser.exit(1, f"FIG player-attack-medium fixture: FAIL: {error}\n")

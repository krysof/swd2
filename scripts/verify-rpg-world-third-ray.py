#!/usr/bin/env python3
"""Replay and lock BUIN1 third-ray interaction and restoration."""
...
def digest(value: object, label: str) -> None:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed BUIN1 third-ray digest {label}")
...
def mapz_oracle(mapz: bytes) -> tuple[list[int], list[int]]:
    ...
    if header != 512 or location != 4608 or area != 23549 or count != 11:
        raise ValueError("BUIN1 third-ray MAPZ structure differs")
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_rpg_buin1_third_ray_interaction_sequence" or \
                ...
                expected.get("empty_center_ray") != \
                    [[17, 170], [16, 170], [15, 170], [14, 170]] or \
                expected.get("empty_second_ray") != \
                    [[17, 171], [16, 171], [15, 171], [14, 171]] or \
                expected.get("selected_side_ray") != 2 or \
                ...
                [page.get("original_review_frame") for page in pages] != \
                    [238, 463, 501, 505, 544]:
            raise ValueError("unsupported RPG BUIN1 third-ray reference")
        ...
        if sha256(release_mapz) != expected["release_mapz_sha256"] or \
            ...
            raise ValueError("release BUIN1 third-ray entity differs")
        ...
        if sha256(fixture_mapz) != expected["fixture_mapz_sha256"] or \
            ...
            raise ValueError("staged BUIN1 third-ray entity differs")
        ...
        if sha256((args.save_root / "SAVE.DA1").read_bytes()) != save_digest or \
            ...
            raise ValueError("RPG BUIN1 third-ray save triplets differ")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError("RPG BUIN1 third-ray input evidence differs")
        if expected.get("capture_wait_seconds") != 6.5 or \
            ...
            raise ValueError("RPG BUIN1 third-ray capture boundary differs")
        for name in (
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("RPG BUIN1 third-ray replay timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    expected["rewrite_mapz_fnv1a64"],
                    expected["rewrite_name_fnv1a64"]):
            raise ValueError("RPG BUIN1 third-ray final state differs")
        if trace.get("transitions") != [{
                "module": "RPG.EXE", "input": "OC", "output": "--",
                "launched": True}]:
            raise ValueError("RPG BUIN1 third-ray replay missed OC entry")

        frames = load_indexed_frames(frame_path)
        if len(frames) != 6:
            raise ValueError("RPG BUIN1 third-ray frame count differs")
        for page in pages:
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "RPG BUIN1 third-ray page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")
        print(
            "RPG BUIN1 third-ray: empty centre and second rays, selected north third ray, entity "
            "event entry and facing restoration match the untouched original "
            "in five complete RGB pages")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"RPG BUIN1 third-ray: FAIL: {error}\n")

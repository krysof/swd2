#!/usr/bin/env python3
"""Replay reachable ability 16 and lock effect 5ah's complete stable RGB sequence."""
...
def digest(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(f"malformed player-effect-5a evidence digest {label}")
    return value
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                ...
                expected.get("ability_id") != 16 or \
                expected.get("effect_code") != 0x5A:
            raise ValueError("unsupported FIG player-effect-5a reference")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError("FIG.EXE differs from the player-effect-5a reference")
        autotype = args.reference.with_name(expected["capture_autotype"])
        if sha256(autotype.read_bytes()) != expected["capture_autotype_sha256"]:
            raise ValueError("original player-effect-5a input differs")
        if sha256((args.save_dir / "SAVE.DA1").read_bytes()) != \
                expected["fixture_save_sha256"]:
            raise ValueError("FIG player-effect-5a staged save differs")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError("FIG player-effect-5a input differs")
        if trace.get("video") != expected["rewrite_video"] or \
                trace.get("frame_fnv1a64", [])[-1:] != [
                    expected["rewrite_final_fnv1a64"]]:
            raise ValueError("FIG player-effect-5a video differs")
        if trace.get("delay_milliseconds") != \
                expected["rewrite_delay_milliseconds"] or \
                trace.get("audio") != expected["rewrite_audio"]:
            raise ValueError("FIG player-effect-5a timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError("FIG player-effect-5a final state differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError("FIG player-effect-5a did not use IF entry")
        ...
        if len(frames) != expected["rewrite_video"]["frames"] or \
            ...
            raise ValueError("FIG player-effect-5a page set differs")
        previous_rewrite = -1
        previous_original = -1
        for page in pages:
            if page["rewrite_frame"] <= previous_rewrite or \
                    page["original_review_frame"] <= previous_original:
                raise ValueError("FIG player-effect-5a page order differs")
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    f"FIG player-effect-5a {page['kind']} no longer matches original")
        ...
        if not isinstance(limitation, str) or "DAC" not in limitation:
            raise ValueError("FIG player-effect-5a capture limitation missing")
        print(
            "FIG player effect 5a checkpoint: 50 stable pose/SP090/damage/"
            "restoration/tail pages exactly match original RGB"
        )
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(1, f"FIG player effect 5a checkpoint: FAIL: {error}\n")

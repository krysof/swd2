#!/usr/bin/env python3
"""Replay FIG's nonfatal player attack followed by resisted monster ability 117."""
...
EXPECTED_KINDS = (
    ...
    *(f"player_damage_rise{index}" for index in range(1, 11)),
    ...
    "monster_special_resisted_prepare_bare",
    "monster_special_resisted_actor_page",
    "monster_special_resisted_cleanup_bare",
...
def digest(value: object, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise ValueError(
            f"malformed FIG player-attack-special-resisted digest {label}")
    return value
...
def main() -> int:
    ...
    try:
        ...
        if expected.get("schema_version") != 1 or \
                expected.get("kind") != \
                    "original_fig_player_attack_special_resisted" or \
                ...
                expected.get("actor_status_immunity") != 1 or \
                ...
                tuple(page.get("rewrite_frame") for page in pages) != \
                    EXPECTED_REWRITE_FRAMES:
            raise ValueError(
                "unsupported FIG player-attack-special-resisted reference")
        if sha256((args.game / "FIG.EXE").read_bytes()) != \
                expected["reference_program_sha256"]:
            raise ValueError(
                "FIG.EXE differs from player-attack-special-resisted reference")
        if sha256((args.game / "SAVE.DA1").read_bytes()) != \
            ...
            raise ValueError("FIG player-attack-special-resisted staged game differs")
        ...
        if sha256(autotype.read_bytes()) != \
            ...
            raise ValueError(
                "FIG player-attack-special-resisted input evidence differs")
        if expected.get("capture_wait_seconds") != 5 or \
            ...
            raise ValueError(
                "FIG player-attack-special-resisted capture boundary differs")
        ...
        if not isinstance(limitation, str) or \
            ...
            raise ValueError(
                "FIG player-attack-special-resisted capture limitation missing")
        ...
        if trace.get("input") != expected["rewrite_input"] or \
            ...
            raise ValueError(
                "FIG player-attack-special-resisted replay boundaries differ")
        if trace.get("video") != expected["rewrite_video"] or \
            ...
            raise ValueError(
                "FIG player-attack-special-resisted audio/video timeline differs")
        if (trace.get("state_fnv1a64"), trace.get("mapz_fnv1a64"),
                trace.get("name_fnv1a64")) != (
                    expected["rewrite_state_fnv1a64"],
                    "827f0f1b725a0958", "e3d2853e2676513b"):
            raise ValueError(
                "FIG player-attack-special-resisted final save triple differs")
        if trace.get("transitions") != [{
                "module": "FIG.EXE", "input": "IF", "output": "--",
                "launched": True}]:
            raise ValueError(
                "FIG player-attack-special-resisted replay did not use IF entry")
        ...
        if frame_times != tuple(expected["rewrite_frame_times_milliseconds"]):
            raise ValueError(
                "FIG player-attack-special-resisted frame timing differs")
        voices = tuple((
        ...
        if voices != ((
            ...
            raise ValueError(
                "FIG player-attack-special-resisted SP003 timeline differs")

        frames = load_indexed_frames(frame_path)
        if len(frames) != expected["rewrite_video"]["frames"]:
            raise ValueError(
                "FIG player-attack-special-resisted frame count differs")
        previous_original = -1
        for page in pages:
            if page["original_review_frame"] <= previous_original:
                raise ValueError(
                    "FIG player-attack-special-resisted original page order differs")
            ...
            if sha256(pixels) != page["rewrite_indexed_sha256"] or \
                ...
                raise ValueError(
                    "FIG player-attack-special-resisted page differs from original: " +
                    page["kind"])
            digest(page.get("original_png_sha256"),
                   page["kind"] + "/original_png_sha256")

        print(
            "FIG player-attack-special-resisted checkpoint: physical poses/wipes, "
            "ten player damage pages, bare-background/pose-2 commit, resisted "
            "20e7 preparation/2721 actor page/22e0 cleanup, round boundary "
            "and next command match 27 original RGB pages")
        return 0
    except (OSError, ValueError, KeyError, IndexError, TypeError,
            json.JSONDecodeError, subprocess.SubprocessError) as error:
        parser.exit(
            1, f"FIG player-attack-special-resisted checkpoint: FAIL: {error}\n")

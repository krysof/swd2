...
竖持手机若不支持浏览器方向锁，游戏仍保持正向的 8:5 横画面并把六个正向触摸按钮放在
下方，绝不把 320×200 像素坐标或按钮文字旋转成侧倒；手机真正转横后才切换为画面左侧、
按钮右侧的布局。按钮禁止文字选择/长按菜单，方向键支持长按，ESC/回车保持单次触发。
页面从后台返回时会重新申请常亮。Wake Lock 只能阻止浏览器可见期间的自动休眠，不能
绕过用户手动锁屏、系统强制省电或浏览器终止后台页面。

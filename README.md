...
竖持手机若不支持浏览器方向锁，CSS 会把完整操作面旋转为横屏方向；横屏视角中方向键
固定在游戏左侧、ESC/回车固定在右侧。canvas 的 CSS 宽高以 `!important` 覆盖
Emscripten 写入的 960×600 inline 尺寸，确保可见游戏始终按 320:200（8:5）完整缩放，
不会在旋转后被裁切或压成狭长条。按钮禁止文字选择/长按菜单，方向键支持长按，ESC/回车
保持单次触发。页面从后台返回时会重新申请常亮。Wake Lock 只能阻止浏览器可见期间的
自动休眠，不能绕过用户手动锁屏、系统强制省电或浏览器终止后台页面。
